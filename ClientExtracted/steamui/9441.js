(self.webpackChunksteamui = self.webpackChunksteamui || []).push([
  [9441],
  {
    73902: (e) => {
      e.exports = {
        DragTarget: "_2sUvh2ZpsDAw1xNqgRBELg",
        DragOnTopOfMe: "_1mvpIyLL0-Pd4QMIoRXHtu",
        DescriptionCtn: "_3DQEBNkYGY3hyLFAjhtq7V",
        EventEditorLinkInput: "_25nbuIEmk-BBWxsSvWGG1n",
        DefaultEditor: "NENu2K19GJmLf1Asga-WF",
        ImportHTMLCtn: "_3sVZHF23hli8ijIwtMs8oU",
        ImportHTMLTextArea: "DHbRFUDVAeXGluFH-smoE",
        ImportHTMLConvertButtons: "lnyZaHhcGtBzDU0SMlFd1",
        ImportHTMLCheckBoxLine: "_3R3FNRLSeiOwBgELGjSPbz",
        OptionRow: "_2Y3MLEmGvWMI8BoNZgCllJ",
        TextEditorToolBarContainer: "_2bOpQtX5QAuQxfGhEJ_iYg",
        EmoteOuter: "_1x3UOXJkizqKhkssRfFjSS",
        YouTubeInput: "_3WXTC22teDkm8BMc01ZTLA",
        YouTubePreviewInsertOption: "_6ocliVvrdQxHPu-upv6-s",
        DropDownScroll: "P0-tbY3743fHY8SAzfF6b",
        InsertSpeakerCtn: "_2f-6Yv5h7xjUcZCrepnQhg",
        TitleGroup: "_1ddLhT39tQNuR4ljq6Nfg5",
        AssociateRowCtn: "_2HeY5m9J-kxRVzGn8dAwv5",
        PhotoCtn: "_2-f4CX_EyXfhRUmPdIey4w",
        AboutCtn: "_3hF9cNUOsfV0BkzEaWn7FM",
        PreviewCtn: "_VCRyh7nyN-2xDV6yH6Sg",
        BBCodeEditorInputStyles: "F506h2OVFDcZeXFtyqthY",
      };
    },
    33724: (e) => {
      e.exports = {
        BBCode_Toggle: "_3dX8-PpYvSNsGv4k5lvP-R",
        Active: "_2vTzhbuJFb9_vHvquo2L-L",
        BBCode: "_1pH9CKzm5VpicOgzyWpsy_",
      };
    },
    67123: (e) => {
      e.exports = { Image: "_1po_jxHTSix3Li3w5ZnMBB" };
    },
    63444: (e) => {
      e.exports = {
        ImagesOuterContainer: "_3A8RGZO2pwg1yKDAdFqp9r",
        Hilight: "_1v_zQLXgFsvon1SwxrWjE-",
        ImageContainer: "_2ti3yMwzfkGoiW68FuNjTG",
        Image: "y902_9A0Wj5bTshbt4xRb",
        ImageFilename: "_2jzLZXXxgDMMcA9X0QDSdg",
      };
    },
    83450: (e) => {
      e.exports = {
        Container: "_2vGr9KsApU0s59_ZsZVxXY",
        SmallAvatar: "_3V8w8CrYHVYB0voJ3gp87x",
        ShareDescription: "_3CqiH5i7kc5lXyEb0TGl2C",
        ShareLink: "_1WTiWXO3q8jMnJc_cPKJbE",
      };
    },
    21501: (e) => {
      e.exports = {
        LinkRegionDragBox: "Rtlc-BB1aJFRIM1lH4zN1",
        EdgeDown: "i9zrHPy0-LgZONeZE4fgG",
        LinkRegionGridBox: "_1Ob4AvWwUMx67yR7owjqse",
        LinkRegionEdge: "_2stP4WlwIxd0-9GjYyI7vF",
        TopLeft: "Clgi---P85XXv25yLZwB0",
        Top: "_2Z9VyBAzofV3JvK__dECbX",
        TopRight: "_2-8DbI8PAEkk6i_0CoUeKM",
        Left: "_3ZwUw4ojIRguwHHAcn2Y4y",
        Middle: "_1HecozzoSZfUZSci9dLkxN",
        LinkRegionDelete: "_3Hb3w5_ECwPKcEr5QSAsNk",
        LinkRegionSettings: "VazMl4niFnodlVJhHIGlL",
        Right: "_3h5fKwHq9Uj2VGs8qxxtLl",
        BottomLeft: "_2CQe0cOBOLqq6y6KAUXqH3",
        Bottom: "sIHlK9sN2255-irERXD_V",
        BottomRight: "_3lnwjSWK9Gh1dFkD46NTpP",
        topleft: "_3W096h6Ka6U7sOZVa9lXQo",
        top: "_1iRW1Msfh60zHqD-xe4EAk",
        topright: "_1Yrl7AkNVVGwbM2vyL8yY1",
        left: "_2iBrmAEyXuaKAeZ-g-4CPF",
        right: "_15t6A4l27DY4KRL1aAUTTS",
        bottomleft: "_3SdBcnCBApw0fQ886qgsUx",
        bottom: "_2kzZ9Ilwo92sEI9LXTtZjN",
        bottomright: "_2AKXkFPsIBpG-HeeN58Rti",
        middle: "_1CS75ZrrDXna6xatw5ZvPR",
        LinkRegionButtonContainer: "_1ZJ42NPmBFvIcOai51ZKv3",
        DialogButton: "nN2Q1qGmO2BGMhVnIVMce",
        LinkRegionInfo: "_3TiV7d40PX30wy8UghFCaJ",
        LinkText: "_2TAc2iPcWUHTtwlg7urHv8",
      };
    },
    3083: (e) => {
      e.exports = {
        WhitelistCtn: "_1UhmxrINvvaNnHzhCPoill",
        WhitelistRow: "_28TC1EYm0jlWPjyk89xXCL",
        WhitelistNumber: "IY3dF3eWXX1OmE8oYcQKp",
        Disabled: "_2VzE-3UQEHXyAext8t7gLW",
        Grabbing: "_1vSZ5gJndAOamRhVGni8HG",
        DragActive: "_31uDZXKZQlYMd8FK9xdaJb",
        Dropped: "_3bfDVSvzMDkk4s1j0Vw8jI",
        JumpToSection: "oABTo2lkoYYI5YMYaeq_Q",
        BeingDragged: "_3y7I4DL9Hua5OhZ4HgcBB5",
        DragGhost: "_61nYWo98IhSjR8PWtQX9O",
        Grabbable: "riuelIz655g_IBddWfLQ-",
        DisabledGrab: "_2K0C_m1AZvB6yeNaEXXjDD",
        WhitelistAvatar: "_3DGjmH9KW9BAXsEYwH1WpE",
        ButtonCtn: "_1hSqlvDTyj9P6eWTHXutUt",
        DragHighlightContainer: "_2jRMC5JVSK6dsktYus9Gjf",
        DragHighlight: "Y9ryg1Npznt3dpkr7BGp1",
      };
    },
    46300: (e) => {
      e.exports = {
        CropImage: "_3qfqTaQ35U6AO3FNeijcFV",
        CropPreviewGroup: "_1RI-QM2ZjK9MaVjeCLE_LF",
        CropPreviewLabel: "_3_zyLDUyxZNyexfX3kNOPv",
      };
    },
    19441: (e, t, i) => {
      "use strict";
      i.r(t), i.d(t, { default: () => Tt });
      var s = i(34629),
        n = i(62540),
        a = i(63696),
        o = i(44846),
        r = i(76565),
        l = i.n(r),
        c = i(4816),
        h = i(90765),
        p = i(46108),
        g = i(51115),
        d = i(72476);
      var u = i(6367),
        m = i(99447),
        I = i(73674),
        C = i(10606),
        L = i(98995),
        T = i(43397),
        w = i(49946),
        A = i.n(w),
        x = i(41230),
        _ = i(28987),
        S = i(89193),
        M = i(53833),
        D = i(93960),
        y = i(90039);
      class b {
        m_eCurLang = (0, o.sf)(d.TS.LANGUAGE);
        m_rgHasData = (0, M.$Y)([], 31, !1);
        m_bHasLocalizationContext = !1;
        m_callback = new y.l();
        GetCallback() {
          return this.m_callback;
        }
        GetCurEditLanguage() {
          return this.m_eCurLang;
        }
        SetCurEditLanguage(e) {
          return (
            this.m_eCurLang != e &&
            ((this.m_eCurLang = e), this.GetCallback().Dispatch(e), !0)
          );
        }
        SetHasLanguage(e) {
          e.forEach((e, t) => {
            this.m_rgHasData[t] != e && (this.m_rgHasData[t] = e);
          });
        }
        BHasLanguageData(e) {
          return this.m_rgHasData[e];
        }
        GetHasLocalizationContext() {
          return this.m_bHasLocalizationContext;
        }
        SetHasLocalizationContext(e) {
          e != this.m_bHasLocalizationContext &&
            (this.m_bHasLocalizationContext = e);
        }
        static s_globalSingletonStore;
        static Get() {
          return (
            b.s_globalSingletonStore ||
              ((b.s_globalSingletonStore = new b()),
              "dev" == d.TS.WEB_UNIVERSE &&
                (window.DUS = b.s_globalSingletonStore)),
            b.s_globalSingletonStore
          );
        }
        constructor() {
          (0, S.Gn)(this);
        }
      }
      (0, s.Cg)([S.sH], b.prototype, "m_eCurLang", void 0),
        (0, s.Cg)([S.sH], b.prototype, "m_rgHasData", void 0),
        (0, s.Cg)([S.sH], b.prototype, "m_bHasLocalizationContext", void 0),
        (0, s.Cg)([D.o], b.prototype, "GetCurEditLanguage", null),
        (0, s.Cg)([D.o], b.prototype, "SetCurEditLanguage", null),
        (0, s.Cg)([S.XI.bound], b.prototype, "SetHasLanguage", null),
        (0, s.Cg)([D.o], b.prototype, "BHasLanguageData", null);
      var v = i(11010);
      let f = class extends a.Component {
        GenerateLanguageOptions() {
          let e = [];
          const {
            fnFilterLanguage: t,
            fnLangHasData: i,
            fnLastUpdateRTime: s,
            fnIsLangSupported: a,
          } = this.props;
          this.props.bAllowUnsetOption &&
            e.push(
              (0, n.jsx)(
                "option",
                { value: -1, children: (0, p.we)("#language_selection_none") },
                "langpicker_unset",
              ),
            );
          let r = new Array();
          const c = this.props.realms || [_.TU.k_ESteamRealmGlobal];
          for (const e of p.A0.GetLanguageListForRealms(c)) {
            if (t && !t(e)) continue;
            const i = (0, o.Lg)(e),
              s = (0, p.we)("#Language_" + i),
              n = Boolean(a) && a(e);
            r.push({ eLang: e, sLocName: s, bSupported: n });
          }
          r.sort((e, t) =>
            e.bSupported != t.bSupported
              ? e.bSupported
                ? -1
                : 1
              : e.sLocName.localeCompare(t.sLocName),
          );
          let g = !1;
          for (const t of r) {
            t.bSupported != g &&
              (e.push(
                (0, n.jsx)(
                  "option",
                  {
                    className: l().SupportedGroupLabel,
                    disabled: !0,
                    children: (0, p.we)(
                      t.bSupported
                        ? "#LanguageGroup_Supported"
                        : "#LanguageGroup_Unsupported",
                    ),
                  },
                  t.bSupported ? "SupportedGroup" : "UnsupportedGroup",
                ),
              ),
              (g = t.bSupported));
            const a = i && i(t.eLang),
              o = s && s(t.eLang);
            let r = t.sLocName;
            o &&
              0 !== o &&
              ((r += " "),
              (r += (0, p.we)(
                "#Language_Last_Update",
                (0, p.$z)(o) + " @ " + (0, v.KC)(o, { bForce24HourClock: !1 }),
              ))),
              e.push(
                (0, n.jsx)(
                  "option",
                  {
                    value: t.eLang,
                    className: (0, h.A)(
                      { [l().LanguageWithContent]: a },
                      t.bSupported
                        ? l().SupportedLanguage
                        : l().UnsupportedLanguage,
                    ),
                    children: r,
                  },
                  "langpicker" + t.eLang + (a ? "_hasdata" : ""),
                ),
              );
          }
          return e;
        }
        OnLanguageChange(e) {
          const { fnOnLanguageChanged: t, selectedLang: i } = this.props;
          let s = Number.parseInt(e.currentTarget.value);
          s != i && t && t(s);
        }
        render() {
          const { selectedLang: e, bDisabled: t, strTooltip: i } = this.props;
          let s = this.GenerateLanguageOptions();
          return (0, n.jsx)(L.he, {
            toolTipContent: i,
            children: (0, n.jsx)("select", {
              value: e,
              onChange: this.OnLanguageChange,
              disabled: t,
              children: s,
            }),
          });
        }
      };
      (0, s.Cg)([g.oI], f.prototype, "OnLanguageChange", null),
        (f = (0, s.Cg)([x.PA], f));
      var j = i(83957),
        E = i.n(j),
        N = i(72388),
        k = i(12176),
        O = i(36934),
        B = i(8573),
        R = i(54946),
        U = i(31084),
        P = i(50712),
        G = i(36885),
        H = i(39039),
        z = i(85688),
        F = i(79769);
      class Y {
        constructor() {
          (0, S.Gn)(this),
            "dev" == d.TS.WEB_UNIVERSE && (window.g_ClanImageStore = this);
        }
        m_mapClanToImages = new Map();
        m_mapClanImageLoadPromises = new Map();
        m_imageListChangeCallback = new Map();
        m_mapClanImageLoadState = new Map();
        m_mapImageIDToResolution = new Map();
        BHasImageResolution(e) {
          return this.m_mapImageIDToResolution.has(e.imageid);
        }
        GetImageResolution(e) {
          return this.m_mapImageIDToResolution.get(e.imageid);
        }
        SetImageResolution(e, t) {
          this.m_mapImageIDToResolution.set(e.imageid, t);
        }
        GetImageListCallbackForClanAccountIDInternal(e) {
          return (
            this.m_imageListChangeCallback.has(e) ||
              this.m_imageListChangeCallback.set(e, new F.lu()),
            this.m_imageListChangeCallback.get(e)
          );
        }
        GetImageListCallbackForClanAccountID(e) {
          return this.GetImageListCallbackForClanAccountIDInternal(e);
        }
        m_vecClanImageDragListener = new Array();
        AddClanImageDragListener(e) {
          -1 == this.m_vecClanImageDragListener.indexOf(e) &&
            this.m_vecClanImageDragListener.push(e);
        }
        RemoveClanImageDragListener(e) {
          let t = this.m_vecClanImageDragListener.indexOf(e);
          -1 != t && this.m_vecClanImageDragListener.splice(t, 1);
        }
        GetClanImageDragListener() {
          return this.m_vecClanImageDragListener;
        }
        BHasLoadedClanImages(e) {
          return this.m_mapClanToImages.has(e.GetAccountID());
        }
        async LoadClanImages(e, t, i) {
          const s = e.GetAccountID();
          if (t || !this.m_mapClanImageLoadPromises.has(s)) {
            const n = this.InternalLoadClanImages(e, t, i);
            this.m_mapClanImageLoadPromises.set(s, n);
          }
          return this.m_mapClanImageLoadPromises.get(s);
        }
        async InternalLoadClanImages(e, t, i) {
          let s = e.GetAccountID();
          if (
            ((0, z.wT)(e && 0 != s, "ClanSteamID missing:" + e),
            e && (!this.m_mapClanToImages.has(s) || t))
          ) {
            let t,
              n = {};
            const a =
              d.TS.COMMUNITY_BASE_URL +
              "/gid/" +
              e.ConvertTo64BitString() +
              "/getimages/";
            if (
              ((t = await E().get(a, {
                params: n,
                withCredentials: !0,
                cancelToken: i,
              })),
              t)
            ) {
              for (let e of t.data.images) e.clanAccountID = s;
              (0, S.h5)(() => {
                this.m_mapClanImageLoadState.set(s, { loaded: !0 }),
                  this.m_mapClanToImages.set(s, t.data.images),
                  this.GetImageListCallbackForClanAccountIDInternal(s).Dispatch(
                    t.data.images,
                  );
              });
            }
          }
          return this.m_mapClanToImages.get(s);
        }
        GetLoadState(e) {
          return this.m_mapClanImageLoadState.get(e.GetAccountID());
        }
        GetClanImages(e) {
          return this.GetClanImagesByAccount(e.GetAccountID());
        }
        GetClanImagesByAccount(e) {
          let t = this.m_mapClanToImages.get(e);
          return t || new Array();
        }
        GetFilteredClanImages(e, t) {
          let i = Q.GetClanImages(e);
          return this.GetFilteredClanImagesList(i, t);
        }
        GetFilteredClanImagesList(e, t) {
          if (t && t.trim().length > 0) {
            t = t.trim().toLowerCase();
            let i = new Array();
            for (let s of e)
              s.file_name &&
                s.file_name.toLowerCase().indexOf(t) >= 0 &&
                i.push(s);
            return i;
          }
          return e;
        }
        GetClanImageByID(e, t) {
          let i = e.GetAccountID(),
            s = this.m_mapClanToImages.get(i);
          return s ? s.find((e) => e.imageid == t) : void 0;
        }
        GetClanImageByURL(e, t) {
          let i = e.GetAccountID(),
            s = this.m_mapClanToImages.get(i);
          return s ? s.find((e) => e.thumb_url == t || e.url == t) : void 0;
        }
        GetClanImageByFile(e, t) {
          let i = e.GetAccountID(),
            s = this.m_mapClanToImages.get(i);
          return s ? s.find((e) => e.file_name == t.name) : void 0;
        }
        GetClanImageByImageHash(e, t) {
          let i = e.GetAccountID(),
            s = this.m_mapClanToImages.get(i);
          return s ? s.find((e) => e.image_hash == t) : void 0;
        }
        async DeleteClanImageByID(e, t) {
          let i = { sessionid: d.TS.SESSIONID, imageid: t },
            s = e.GetAccountID(),
            n = await E().get(
              d.TS.COMMUNITY_BASE_URL +
                "/gid/" +
                e.ConvertTo64BitString() +
                "/deleteimage/",
              { params: i },
            );
          if (!n || 200 != n.status || 1 != n.data.success) return n.data;
          let a = this.m_mapClanToImages.get(s);
          if (a) {
            let e = a.findIndex((e, i, s) => e.imageid == t);
            e >= 0 &&
              (a.splice(e, 1),
              this.GetImageListCallbackForClanAccountIDInternal(s).Dispatch([
                ...a,
              ]));
          }
          return n.data;
        }
        async DeleteClanImage(e, t) {
          return this.DeleteClanImageByID(e, t.imageid);
        }
      }
      (0, s.Cg)([S.sH], Y.prototype, "m_mapClanToImages", void 0),
        (0, s.Cg)([S.sH], Y.prototype, "m_mapClanImageLoadState", void 0);
      const Q = new Y();
      var V = i(7372),
        Z = i(98592),
        W = i(33724),
        J = i.n(W),
        X = i(48214),
        K = i.n(X);
      class q extends Z.A {
        performSearch(e) {
          let t = Array();
          return (
            this.props.supportBBCodes.forEach((i) => {
              i.indexOf(e) >= 0 && t.push({ name: i });
            }),
            t
          );
        }
        getSelection(e) {
          return "[" + e.name + "][/" + e.name + "]";
        }
        getKey(e) {
          return e.name;
        }
        renderMatch(e) {
          return (0, n.jsxs)(
            "div",
            {
              className: (0, h.A)(J().BBCodeSuggestion),
              children: [
                (0, n.jsx)("div", { className: J().BBCode, children: e.name }),
                "[",
                e.name,
                "]...[/",
                e.name,
                "]",
              ],
            },
            e.name,
          );
        }
        renderNoMatchMessage() {
          return (0, n.jsx)(
            "div",
            {
              className: (0, h.A)(K().mentionSearchOption, K().noMatches),
              children: (0, p.we)("#Bbcode_No_Match"),
            },
            "nomatches",
          );
        }
        renderTooManyMatchesMessage(e) {
          return null;
        }
        renderHeader() {
          return null;
        }
        getMaxMatches() {
          return Number.MAX_VALUE;
        }
      }
      class $ extends a.Component {
        descTextAreaRef = a.createRef();
        m_MentionDialog;
        m_bDisabled = !0;
        m_iMentionSearchStartOffset;
        m_iMentionSearchCancelledOffset;
        constructor(e) {
          super(e),
            (this.state = {
              mentionSearch: void 0,
              activeSuggestSearchType: void 0,
            });
        }
        BindMentionDialog(e) {
          this.m_MentionDialog = e;
        }
        OnKeyDown(e) {
          this.state.activeSuggestSearchType &&
          this.m_MentionDialog &&
          !e.shiftKey &&
          !e.ctrlKey &&
          this.m_MentionDialog.BHandleKeyPress(e.keyCode)
            ? e.preventDefault()
            : (8 != e.keyCode && 46 != e.keyCode) ||
              (this.m_iMentionSearchCancelledOffset = void 0);
        }
        FindMatchOpener(e, t, i) {
          for (let s = i - 1; s >= 0; s--) {
            if (t[s] == e) return s;
            if (" " == t[s] || "\n" == t[s]) break;
          }
        }
        ReplaceSuggestedText(e, t) {
          let i = this.descTextAreaRef.current.selectionStart,
            s = this.descTextAreaRef.current.value,
            n = this.FindMatchOpener(e, s, i);
          if (
            (void 0 === n && "@" == e && (n = this.FindMatchOpener("＠", s, i)),
            void 0 !== n)
          ) {
            let e = s.substr(0, n);
            (e += t), (i >= s.length || " " != s[i]) && (e += " ");
            let a = e.length;
            (e += s.substr(i)),
              (this.descTextAreaRef.current.value = e),
              (this.descTextAreaRef.current.selectionStart =
                this.descTextAreaRef.current.selectionEnd =
                  a),
              this.props.fnSetText(e),
              this.FocusTextInput();
          }
        }
        OnFocus(e) {
          this.UpdateAutoSearchState();
        }
        OnKeyPress(e) {
          this.UpdateAutoSearchState();
        }
        OnClick(e) {
          this.UpdateAutoSearchState();
        }
        ClearMentionSearchState() {
          (this.m_iMentionSearchStartOffset = void 0),
            this.state.activeSuggestSearchType &&
              this.setState({
                activeSuggestSearchType: void 0,
                mentionSearch: void 0,
              });
        }
        OnEmoticonSuggestionSelected(e, t) {
          if (!t)
            return (
              (this.m_iMentionSearchCancelledOffset =
                this.m_iMentionSearchStartOffset),
              void this.ClearMentionSearchState()
            );
          this.ReplaceSuggestedText(":", ":" + t + ":"),
            this.ClearMentionSearchState();
        }
        OnBBCodeSuggestionSelected(e, t) {
          if (!t)
            return (
              (this.m_iMentionSearchCancelledOffset =
                this.m_iMentionSearchStartOffset),
              void this.ClearMentionSearchState()
            );
          this.ReplaceSuggestedText("[", t), this.ClearMentionSearchState();
        }
        FocusTextInput() {
          this.descTextAreaRef.current && this.descTextAreaRef.current.focus();
        }
        UpdateAutoSearchState() {
          let e = this.descTextAreaRef.current;
          if (!e || this.m_bDisabled) return;
          if (e.selectionStart != e.selectionEnd || !e.selectionStart)
            return void this.ClearMentionSearchState();
          let t,
            i,
            s = e.selectionStart,
            n = e.value;
          for (let e = s - 1; e >= 0; e--) {
            let s = e > 0 ? n[e - 1] : void 0;
            if (this.props.emoticonStore && ":" == n[e] && n.length > 2) {
              (s && " " != s && "\n" != s && ":" != s) ||
                ((t = e), (i = "Emoticon"));
              break;
            }
            if ("[" == n[e] && (e + 1 > n.length || "/" != n[e + 1])) {
              (s && " " != s && "\n" != s && "]" != s) ||
                ((t = e), (i = "BBCode"));
              break;
            }
            if (" " == n[e] || "\n" == n[e]) break;
          }
          if (void 0 === t || t === this.m_iMentionSearchCancelledOffset)
            return void this.ClearMentionSearchState();
          let a = n.substr(t + 1, s - t - 1);
          (this.m_iMentionSearchStartOffset = t),
            this.setState({ activeSuggestSearchType: i, mentionSearch: a });
        }
        GetTextAreaRef() {
          return this.descTextAreaRef;
        }
        GetTextAreaCurrent() {
          return this.descTextAreaRef.current;
        }
        render() {
          let e,
            {
              emoticonStore: t,
              supportBBCodes: i,
              fnSetText: s,
              ...o
            } = this.props;
          switch (this.state.activeSuggestSearchType) {
            case "Emoticon":
              e = (0, n.jsx)(V.A, {
                emoticonStore: t,
                strSearch: this.state.mentionSearch,
                nMinimumSearchLengthBeforeAutoSelection: 2,
                onSuggestionSelected: this.OnEmoticonSuggestionSelected,
                ref: this.BindMentionDialog,
              });
              break;
            case "BBCode":
              e = (0, n.jsx)(q, {
                supportBBCodes: i,
                strSearch: this.state.mentionSearch,
                nMinimumSearchLengthBeforeAutoSelection: 2,
                onSuggestionSelected: this.OnBBCodeSuggestionSelected,
                ref: this.BindMentionDialog,
              });
          }
          return (0, n.jsxs)(a.Fragment, {
            children: [
              e,
              (0, n.jsx)("textarea", {
                ...o,
                ref: this.descTextAreaRef,
                onKeyDown: this.OnKeyDown,
                onKeyUp: this.OnKeyPress,
                onFocus: this.OnFocus,
                onClick: this.OnClick,
              }),
            ],
          });
        }
      }
      (0, s.Cg)([g.oI], $.prototype, "BindMentionDialog", null),
        (0, s.Cg)([g.oI], $.prototype, "OnKeyDown", null),
        (0, s.Cg)([g.oI], $.prototype, "OnFocus", null),
        (0, s.Cg)([g.oI], $.prototype, "OnKeyPress", null),
        (0, s.Cg)([g.oI], $.prototype, "OnClick", null),
        (0, s.Cg)([g.oI], $.prototype, "OnEmoticonSuggestionSelected", null),
        (0, s.Cg)([g.oI], $.prototype, "OnBBCodeSuggestionSelected", null),
        (0, s.Cg)([g.oI], $.prototype, "FocusTextInput", null),
        (0, s.Cg)([g.oI], $.prototype, "GetTextAreaRef", null),
        (0, s.Cg)([g.oI], $.prototype, "GetTextAreaCurrent", null);
      var ee = i(64608),
        te = i(15688),
        ie = i(13869),
        se = i(3083),
        ne = i.n(se),
        ae = i(54644);
      var oe = i(15918),
        re = i(17613),
        le = i(88750),
        ce = i(13352),
        he = i(26853),
        pe = i(73902),
        ge = i.n(pe);
      function de(e) {
        return Array.isArray(e) ? e[0] : e;
      }
      function ue(e) {
        const t = Array.isArray(e) ? e : [e];
        return Math.min(...t);
      }
      function me(e, t) {
        return void 0 === t ? de(e) : Array.isArray(e) ? e[t] : e;
      }
      const Ie = [1, 3, 2],
        Ce = [1, 3],
        Le = [5, 4],
        Te = {
          capsule: { width: 800, height: 450, rgAcceptableTypes: Ce },
          marketingmessage_art_2_eventcapsule: {
            width: 800,
            height: 450,
            rgAcceptableTypes: Ce,
          },
          marketingmessage_art_eventcapsule: {
            width: 800,
            height: 450,
            rgAcceptableTypes: Ce,
          },
          spotlight: { width: 2108, height: 460, rgAcceptableTypes: Ce },
          localized_store_app_spotlight: {
            width: 1200,
            height: 260,
            rgAcceptableTypes: Ce,
          },
          localized_store_app_spotlight_mobile: {
            width: 500,
            height: 160,
            rgAcceptableTypes: Ce,
          },
          localized_title_image: {
            width: 1920,
            height: 622,
            rgAcceptableTypes: Ce,
          },
          background: { width: 1920, height: 622, rgAcceptableTypes: Ce },
          hero: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ce,
          },
          email_full: { width: 800, height: 300, rgAcceptableTypes: Ce },
          email_centered: { width: 644, height: 300, rgAcceptableTypes: Ce },
          broadcast_left: {
            width: [199, 155],
            height: [433, 337],
            rgAcceptableTypes: Ce,
          },
          broadcast_right: {
            width: [199, 155],
            height: [433, 337],
            rgAcceptableTypes: Ce,
          },
          sale_header: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ie,
          },
          sale_overlay: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ie,
          },
          localized_image_group: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ce,
          },
          localized_background_art: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ce,
          },
          sale_section_background: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ie,
          },
          sale_section_title: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ie,
          },
          link_capsule: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ce,
          },
          product_banner: {
            width: [1200, 1100],
            height: [175, 160],
            rgAcceptableTypes: Ce,
          },
          product_mobile_banner: {
            width: 500,
            height: 160,
            rgAcceptableTypes: Ce,
          },
          product_banner_override: {
            width: [1200, 1100],
            height: [175, 160],
            rgAcceptableTypes: Ce,
          },
          product_mobile_banner_override: {
            width: 500,
            height: 160,
            rgAcceptableTypes: Ce,
          },
          schedule_track_art: { width: 196, height: 92, rgAcceptableTypes: Ce },
          tab_bar_background: {
            width: 1500,
            height: 100,
            rgAcceptableTypes: Ce,
          },
          sale_logo: {
            width: [1200, 940],
            height: [460, 460],
            rgAcceptableTypes: Ce,
          },
          bestofyear_banner: {
            width: 1100,
            height: 160,
            rgAcceptableTypes: Ie,
          },
          bestofyear_banner_mobile: {
            width: 500,
            height: 160,
            rgAcceptableTypes: Ie,
          },
          localized_marketing_message: {
            width: 570,
            height: 600,
            rgAcceptableTypes: [1, 3, 2, 10],
          },
          localized_optin_banner: {
            width: 1e3,
            height: 150,
            rgAcceptableTypes: Ce,
          },
          localized_marketingmessage_webm: {
            width: 570,
            height: 600,
            rgAcceptableTypes: [5],
          },
          localized_marketingmessage_mp4: {
            width: 570,
            height: 600,
            rgAcceptableTypes: [4],
          },
          localized_partnerevent_webm: {
            width: 800,
            height: 450,
            rgAcceptableTypes: [5],
          },
          localized_partnerevent_mp4: {
            width: 800,
            height: 450,
            rgAcceptableTypes: [4],
          },
          localized_subtitles: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: [6, 7],
          },
          localized_marketingmessage_poster: {
            width: 528,
            height: 297,
            rgAcceptableTypes: [1, 3],
          },
          localized_marketingmessage_background: {
            width: 570,
            height: 600,
            rgAcceptableTypes: Ce,
          },
          localized_email_image: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ce,
          },
          template_asset: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: [1, 3, 2, 10, 5, 4],
          },
          user_poll_background: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: Ce,
          },
          spotlight_art: { width: 306, height: 260, rgAcceptableTypes: Ie },
          spotlight_art_hero: {
            width: 374,
            height: 448,
            rgAcceptableTypes: Ce,
          },
          old_spotlight_art: { width: 306, height: 350, rgAcceptableTypes: Ie },
          marketingmessage_art: {
            width: 570,
            height: 600,
            rgAcceptableTypes: Ie,
          },
          marketingmessage_art_2: {
            width: 570,
            height: 600,
            rgAcceptableTypes: Ie,
          },
          takeover_art: { width: 1850, height: 450, rgAcceptableTypes: Ie },
          takeover_webm_art: {
            width: 1850,
            height: 450,
            rgAcceptableTypes: [5],
          },
          takeover_mp4_art: {
            width: 1850,
            height: 450,
            rgAcceptableTypes: [4],
          },
          takeover_mobile_art: {
            width: 500,
            height: 350,
            rgAcceptableTypes: Ie,
          },
          takeover_webm_mobile_art: {
            width: 500,
            height: 350,
            rgAcceptableTypes: [5],
          },
          takeover_mp4_mobile_art: {
            width: 500,
            height: 350,
            rgAcceptableTypes: [4],
          },
          takeunder_art: { width: 1200, height: 190, rgAcceptableTypes: Ie },
          takeunder_mobile_art: {
            width: 500,
            height: 160,
            rgAcceptableTypes: Ie,
          },
          app_header_capsule: {
            width: 920,
            height: 430,
            rgAcceptableTypes: Ce,
          },
          app_main_capsule: { width: 1232, height: 706, rgAcceptableTypes: Ce },
        };
      function we(e, t, i, s) {
        let n = null;
        if (Array.isArray(i)) {
          if (
            ((n = i
              .map((t, i) => (e === t ? i : void 0))
              .filter((e) => void 0 !== e)),
            n.length <= 0)
          )
            return !1;
        } else if (e !== i) return !1;
        if (Array.isArray(s)) {
          const e = s
            .map((e, i) => (t === e ? i : void 0))
            .filter((e) => void 0 !== e);
          if (e.length <= 0) return !1;
          if (n?.length && !e.some((e) => n.includes(e))) return !1;
        } else if (t !== s) return !1;
        return !0;
      }
      function Ae(e, t, i, s) {
        const n = Te[i];
        return (
          !!n &&
          (n.bDisableEnforceDimensions ? !!s : we(e, t, n.width, n.height))
        );
      }
      function xe(e, t, i) {
        const s = Te[i];
        if (!s) return !1;
        if (s.bDisableEnforceDimensions) return !0;
        if (Array.isArray(s.width)) {
          if (s.width.filter((t) => e < t).length == s.width.length) return !1;
        } else if (e < s.width) return !1;
        if (Array.isArray(s.height)) {
          if (s.height.filter((e) => t < e).length == s.height.length)
            return !1;
        } else if (t < s.height) return !1;
        return !0;
      }
      function _e(e, t) {
        return t.filter((t) => Se(e, t));
      }
      function Se(e, t) {
        return Te[t].rgAcceptableTypes.includes(e);
      }
      var Me = i(29381),
        De = i(46300);
      class ye extends a.Component {
        state = {
          region: {
            xPosPct: 0,
            yPosPct: 0,
            widthPct: (this.GetDestWidth() / this.props.uploadFile.width) * 100,
            heightPct:
              (this.GetDestHeight() / this.props.uploadFile.height) * 100,
          },
        };
        async OnCrop() {
          const e = this.props.uploadFile.GetCanvasImageSource();
          e &&
            (await (async function (e, t, i, s, n, a, o, r, l) {
              return new Promise((c, h) => {
                const p = be(l);
                if (!p) return void h("Invalid format provided");
                const g = document.createElement("canvas");
                (g.width = o), (g.height = r);
                const d = 0,
                  u = 0,
                  m = g.getContext("2d");
                m?.drawImage(t, i, s, n, a, d, u, o, r),
                  g.toBlob((t) => {
                    const i = g.toDataURL(p);
                    3 !== l && i.startsWith("data:image/png")
                      ? h("Unable to encode into the requested file format")
                      : t
                        ? ((e.file = (0, ae.pE)(t, e.filename)),
                          (e.width = o),
                          (e.height = r),
                          (e.dataUrl = i),
                          (e.uploadTime = Date.now()),
                          (e.bCropped = !0),
                          c())
                        : h("Unable to apply crop into image");
                  });
              });
            })(
              this.props.uploadFile,
              e,
              (this.state.region.xPosPct / 100) * this.props.uploadFile.width,
              (this.state.region.yPosPct / 100) * this.props.uploadFile.height,
              (this.state.region.widthPct / 100) * this.props.uploadFile.width,
              (this.state.region.heightPct / 100) *
                this.props.uploadFile.height,
              this.GetDestWidth(),
              this.GetDestHeight(),
              this.props.fileType,
            )),
            this.props.closeModal?.();
        }
        UpdateCrop(e, t) {
          this.setState({ region: t });
        }
        GetDestWidth() {
          const { uploadFile: e, forceResolution: t } = this.props;
          if (t) return t.width;
          const i = e.GetCurrentImageOption();
          if (!i) return 0;
          const s = Te[i.artworkType].width;
          return i ? de(s) : 0;
        }
        GetDestHeight() {
          const { uploadFile: e, forceResolution: t } = this.props;
          if (t) return t.width;
          const i = e.GetCurrentImageOption();
          if (!i) return 0;
          const s = Te[i.artworkType].height;
          return i ? de(s) : 0;
        }
        GetLargestBoxThatFits(e, t, i, s) {
          let n = i,
            a = (n * t) / Math.max(e, 1);
          return (
            a > s && ((a = s), (n = (a * e) / Math.max(t, 1))),
            { width: n, height: a }
          );
        }
        GetPreviewWindowStyle() {
          const { region: e } = this.state,
            t = this.GetLargestBoxThatFits(
              this.GetDestWidth(),
              this.GetDestHeight(),
              500,
              150,
            ),
            i = t.width,
            s = t.height,
            n = 1 / Math.max(e.widthPct / 100, 1e-4),
            a = 1 / Math.max(e.heightPct / 100, 1e-4),
            o = (this.props.uploadFile.width * e.xPosPct) / 100,
            r = (this.props.uploadFile.height * e.yPosPct) / 100;
          return {
            width: i,
            height: s,
            backgroundPosition: `${-o * ((i * n) / this.props.uploadFile.width)}px ${-r * ((s * a) / this.props.uploadFile.height)}px`,
            backgroundSize: `${100 * n}% ${100 * a}%`,
            backgroundImage: `url(${this.props.uploadFile.dataUrl})`,
          };
        }
        render() {
          const e = (this.GetDestWidth() / this.props.uploadFile.width) * 100,
            t = (this.GetDestHeight() / this.props.uploadFile.height) * 100,
            i = this.GetLargestBoxThatFits(
              this.props.uploadFile.width,
              this.props.uploadFile.height,
              800,
              500,
            );
          return (0, n.jsx)(C.x_, {
            onEscKeypress: this.props.closeModal,
            bDisableBackgroundDismiss: !0,
            children: (0, n.jsxs)("div", {
              className: (0, h.A)("DialogContent", "_DialogCenterVertically"),
              children: [
                (0, n.jsx)(ee.iK, {
                  children: (0, p.we)(
                    "#ImageUpload_CropModalTitleDims",
                    this.GetDestWidth(),
                    this.GetDestHeight(),
                  ),
                }),
                (0, n.jsx)("div", {
                  className: (0, h.A)("DialogBodyText"),
                  children: (0, p.we)("#ImageUpload_CropModalDescription"),
                }),
                (0, n.jsxs)("div", {
                  className: De.CropImage,
                  style: { width: i.width, height: i.height },
                  children: [
                    (0, n.jsx)("img", {
                      style: {
                        maxWidth: "100%",
                        maxHeight: "100%",
                        objectFit: "contain",
                      },
                      src: this.props.uploadFile.dataUrl,
                    }),
                    (0, n.jsx)(Me.I, {
                      bLockAspectRatio: !0,
                      bDisableLink: !0,
                      index: 0,
                      updateFn: this.UpdateCrop,
                      xPosPct: 0,
                      yPosPct: 0,
                      widthMinPct: e,
                      heightMinPct: t,
                      widthPct: e,
                      heightPct: t,
                    }),
                  ],
                }),
                (0, n.jsxs)("div", {
                  className: De.CropPreviewGroup,
                  children: [
                    (0, n.jsx)("div", {
                      className: De.CropPreviewLabel,
                      children: (0, p.we)("#ImageUpload_CropPreview"),
                    }),
                    (0, n.jsx)("div", { style: this.GetPreviewWindowStyle() }),
                  ],
                }),
                (0, n.jsx)(ee.jn, {
                  onClick: this.OnCrop,
                  children: (0, p.we)("#ImageUpload_CropAndContinue"),
                }),
              ],
            }),
          });
        }
      }
      function be(e) {
        switch (e) {
          case 3:
            return "image/png";
          case 1:
            return "image/jpeg";
        }
      }
      (0, s.Cg)([g.oI], ye.prototype, "OnCrop", null),
        (0, s.Cg)([g.oI], ye.prototype, "UpdateCrop", null);
      var ve = i(42318),
        fe = i(63444);
      const je = (e) => {
          const { clanSteamID: t, fnImageSelectCallBack: i } = e,
            [s, o] = (0, a.useState)(""),
            r = (function (e) {
              const t = B.b.InitFromClanID(e),
                i = (0, H.m)("useLoadClanImages"),
                [s, n] = (0, a.useState)(() => Q.BHasLoadedClanImages(t));
              return (
                (0, a.useEffect)(() => {
                  const t = B.b.InitFromClanID(e);
                  return (
                    Q.BHasLoadedClanImages(t) ||
                      Q.LoadClanImages(t, !1, i.token).then(() => n(!0)),
                    () => i.cancel()
                  );
                }, [e, i]),
                s
              );
            })(e.clanSteamID.GetAccountID()),
            l = () => e.closeModal && e.closeModal(),
            c = Q.GetFilteredClanImages(t, s),
            h = (e) => {
              i(e), l();
            };
          return (0, n.jsx)(ve.tH, {
            children: (0, n.jsx)(C.x_, {
              onEscKeypress: l,
              children: (0, n.jsxs)(ee.UC, {
                children: [
                  (0, n.jsx)(ee.Y9, {
                    children: (0, p.we)("#ClanImageChooser_Title"),
                  }),
                  (0, n.jsx)(ee.nB, {
                    children: (0, n.jsxs)(ee.a3, {
                      children: [
                        (0, n.jsx)("p", {
                          children: (0, p.we)("#ClanImageChooser_Desc"),
                        }),
                        (0, n.jsx)(ee.pd, {
                          placeholder: (0, p.we)("#ClanImageChooser_Search"),
                          value: s,
                          onChange: (e) => o(e.currentTarget.value),
                        }),
                        (0, n.jsx)("div", {
                          className: fe.ImagesOuterContainer,
                          children: r
                            ? (0, n.jsx)(he.t, {
                                size: "medium",
                                string: (0, p.we)("#Loading"),
                              })
                            : Boolean(c.length > 0)
                              ? c.map((e) =>
                                  (0, n.jsx)(
                                    Ee,
                                    {
                                      clanImage: e,
                                      searchStringHilight: s,
                                      fnImageClick: h,
                                    },
                                    "ci" + e.image_hash,
                                  ),
                                )
                              : Boolean(0 == s.trim().length)
                                ? (0, n.jsx)("div", {
                                    children: (0, p.we)(
                                      "#ClanImageChooser_None",
                                    ),
                                  })
                                : (0, n.jsx)("div", {
                                    children: (0, p.we)(
                                      "#EventCalendar_GameSearch_NoneFound",
                                    ),
                                  }),
                        }),
                      ],
                    }),
                  }),
                  (0, n.jsx)(ee.wi, {
                    children: (0, n.jsx)(ee.$n, {
                      onClick: l,
                      children: (0, p.we)("#Button_Cancel"),
                    }),
                  }),
                ],
              }),
            }),
          });
        },
        Ee = (e) => {
          const { clanImage: t, searchStringHilight: i, fnImageClick: s } = e;
          let a = t.file_name ? t.file_name : "",
            o = (function (e, t, i, s) {
              let a = [];
              if (e.length > 0) {
                let o = t.toLocaleLowerCase();
                for (let r = 0; r < t.length; ) {
                  let l = o.indexOf(e, r);
                  if (l < 0) {
                    a.push(
                      (0, n.jsx)(
                        "span",
                        { children: t.substring(r) },
                        i + "_" + String(r),
                      ),
                    );
                    break;
                  }
                  r < l &&
                    a.push(
                      (0, n.jsx)(
                        "span",
                        { children: t.substring(r, l) },
                        i + "_" + String(r),
                      ),
                    ),
                    a.push(
                      (0, n.jsx)(
                        "span",
                        { className: s, children: t.substr(l, e.length) },
                        i + "_" + String(r),
                      ),
                    ),
                    (r = l + e.length);
                }
              } else a.push((0, n.jsx)("span", { children: t }, i + "_null"));
              return a;
            })(i, a, String(t.imageid), fe.Hilight);
          return (0, n.jsxs)("div", {
            className: fe.ImageContainer,
            children: [
              (0, n.jsx)("div", {
                className: fe.Image,
                style: { backgroundImage: `url( '${t.thumb_url}' )` },
                onDoubleClick: () => s(t),
              }),
              (0, n.jsx)("div", {
                className: fe.ImageFilename,
                title: a,
                children: o,
              }),
            ],
          });
        };
      var Ne = i(67123),
        ke = i(42898),
        Oe = i(16154),
        Be = i(79048),
        Re = i(21920);
      class Ue {
        m_originalSize = { width: 0, height: 0 };
        m_originalDataUrl = "";
        dataUrl = void 0;
        width = 0;
        height = 0;
        status = "pending";
        message = "";
        language = void 0;
        file;
        filename;
        uploadTime;
        fileType;
        constructor(e, t, i, s, n) {
          (0, S.Gn)(this),
            (this.file = e),
            (this.filename = t),
            (this.fileType = (0, Re.yh)(t) ?? 0),
            (this.language = i),
            (this.uploadTime = Date.now()),
            (this.status = "pending"),
            (this.m_originalSize = n),
            (this.height = n.height),
            (this.width = n.width),
            (this.m_originalDataUrl = s),
            (this.dataUrl = s);
        }
        ResetImage() {
          (this.height = this.m_originalSize.height),
            (this.width = this.m_originalSize.width),
            (this.dataUrl = this.m_originalDataUrl);
        }
        GetImageOptionLabel() {}
      }
      (0, s.Cg)([S.sH], Ue.prototype, "dataUrl", void 0),
        (0, s.Cg)([S.sH], Ue.prototype, "width", void 0),
        (0, s.Cg)([S.sH], Ue.prototype, "height", void 0),
        (0, s.Cg)([S.sH], Ue.prototype, "status", void 0),
        (0, s.Cg)([S.sH.ref], Ue.prototype, "message", void 0),
        (0, s.Cg)([S.sH], Ue.prototype, "language", void 0);
      class Pe extends Ue {
        m_rgImageOptions;
        m_currentImageOption = void 0;
        m_currentImageOptionKey = void 0;
        constructor(e, t, i, s, n, a) {
          super(e, t, i, n, a), (0, S.Gn)(this), (this.m_rgImageOptions = s);
        }
        IsValidAssetType(e, t) {
          let i = 0,
            s = 0,
            n = !1,
            a =
              !this.m_rgImageOptions ||
              0 === this.m_rgImageOptions.length ||
              this.m_rgImageOptions.some(
                (e) => e.sKey == this.GetCurrentImageOption()?.sKey,
              );
          if (e) (i = e.width), (s = e.height), (n = !0);
          else if (this.GetCurrentImageOption()) {
            const e = Te[this.GetCurrentImageOption().artworkType];
            e &&
              ((i = e.width),
              (s = e.height),
              (n = !e.bDisableEnforceDimensions));
          }
          const o = this.width >= ue(i) && this.height >= ue(s),
            r = n ? we(this.width, this.height, i, s) : o,
            l = t && t != this.fileType,
            c =
              !!(this.m_rgImageOptions && this.m_rgImageOptions.length > 0) &&
              0 ==
                _e(
                  this.fileType,
                  this.m_rgImageOptions?.map((e) => e.artworkType) || [],
                ).length,
            h = Boolean(be(this.fileType));
          let g,
            d = "",
            u = !1;
          return (
            a
              ? c
                ? (d = (0, p.we)("#ImageUpload_InvalidFileType"))
                : l
                  ? (d = (0, p.we)(
                      "#ImageUpload_InvalidFormat",
                      (0, Be.EG)(t) ?? "",
                    ))
                  : r || h
                    ? o
                      ? !r && h
                        ? ((d = (0, p.we)(
                            "#ImageUpload_InvalidDimensions",
                            de(i),
                            de(s),
                          )),
                          (u = !0))
                        : ((Array.isArray(i) && this.width != de(i)) ||
                            (Array.isArray(s) && this.height != de(s))) &&
                          ((g = g ?? []),
                          g.push(
                            (0, p.we)(
                              "#ImageUpload_PreferredDimension",
                              de(i),
                              de(s),
                            ),
                          ))
                      : (d = (0, p.we)("#ImageUpload_TooSmall", de(i), de(s)))
                    : (d = (0, p.we)(
                        "#ImageUpload_InvalidResolution",
                        de(i),
                        de(s),
                      ))
              : (d = (0, p.we)("#ImageUpload_InvalidFormatSelected")),
            {
              error: d,
              warnings: g,
              needsCrop: u,
              match: this.GetCurrentImageOption(),
            }
          );
        }
        BSupportsLanguages() {
          return !0;
        }
        ComputeDefaultImageOption() {
          if (!this.m_rgImageOptions || 0 == this.m_rgImageOptions.length)
            return;
          const e = _e(
            this.fileType,
            this.m_rgImageOptions?.map((e) => e.artworkType),
          );
          let t = He(this.width, this.height, e, !1);
          if ((void 0 === t && (t = He(this.width, this.height, e, !0)), t)) {
            const e = this.m_rgImageOptions.find(
              (e) =>
                e.artworkType == t &&
                (!e.bEnforceDimensions ||
                  (e.width == this.width && e.height == this.height)),
            );
            if (e) return e;
          }
          return this.m_rgImageOptions[0];
        }
        get ImageOptions() {
          return this.m_rgImageOptions;
        }
        GetCurrentImageOptionKey() {
          return this.m_currentImageOptionKey;
        }
        GetCurrentImageOption() {
          return this.m_currentImageOption ?? this.ComputeDefaultImageOption();
        }
        SetCurrentImageOption(e) {
          (this.m_currentImageOption = e),
            (this.m_currentImageOptionKey = e?.sKey);
        }
      }
      (0, s.Cg)([S.sH], Pe.prototype, "m_currentImageOption", void 0),
        (0, s.Cg)([S.sH], Pe.prototype, "m_currentImageOptionKey", void 0);
      class Ge extends Pe {
        bCropped = !1;
        localizedImageGroupPrimaryImage;
        media;
        constructor(e, t, i, s, n, a, o, r) {
          super(e, t, i, s, n, a),
            (0, S.Gn)(this),
            (this.media = o),
            (this.localizedImageGroupPrimaryImage = r);
        }
        IsValidAssetType(e, t) {
          return (
            (t = t ?? this.localizedImageGroupPrimaryImage?.file_type),
            super.IsValidAssetType(e, t)
          );
        }
        GetCanvasImageSource() {
          return this.media;
        }
        BIsOriginalMinimumDimensions(e) {
          return xe(
            this.media?.width ?? 0,
            this.media?.height ?? 0,
            e.artworkType,
          );
        }
        FileTypeMatchesImageTypes(e) {
          return Se(this.fileType, e.artworkType);
        }
        BIsVideo() {
          return Le.includes(this.fileType);
        }
        GetResizeDimension() {
          return (function (e) {
            if ("background" === e)
              return [
                { width: 960, height: 311 },
                { width: 480, height: 156 },
              ];
            if ("capsule" === e)
              return [
                { width: de(Te[e].width) / 2, height: de(Te[e].height) / 2 },
              ];
            if ("spotlight" === e)
              return [
                { width: de(Te[e].width) / 2, height: de(Te[e].height) / 2 },
              ];
            return;
          })(this.GetCurrentImageOption()?.artworkType);
        }
      }
      function He(e, t, i, s = !1) {
        if (i)
          for (let n of i) {
            if (s ? xe(e, t, n) : Ae(e, t, n)) return n;
          }
      }
      (0, s.Cg)([S.sH], Ge.prototype, "bCropped", void 0);
      class ze {}
      function Fe(e, t, i) {
        const s = e.filter((e) => {
          const s = e.IsValidAssetType(t, i);
          return "pending" === e.status && !s.error && !s.needsCrop;
        });
        return (
          s.forEach((e) => {
            (e.status = "waiting"), (e.message = "");
          }),
          s
        );
      }
      async function Ye(e, t, i, s, n, a) {
        const o = Fe(e, s, n),
          r = [];
        let l = 0;
        const c = Array.from({ length: Math.floor(t) }, () =>
          (async () => {
            for (; l < o.length; ) {
              const e = l++,
                t = o[e];
              t.status = "uploading";
              const s = await i(t, t.filename, t.language ?? -1, a);
              (t.status = s.bSuccess ? "success" : "failed"),
                (t.message =
                  !s.bSuccess && s.elErrorMessage ? s.elErrorMessage : ""),
                (r[e] = { image: t, uploadResult: s });
            }
          })(),
        );
        return (
          await Promise.all(c),
          r.map((e) => ({
            bSuccess: e.uploadResult.bSuccess,
            image: e.image,
            uploadResult: e.uploadResult.result,
          }))
        );
      }
      class Qe extends ze {
        m_cancel = void 0;
        async UploadAllImages(e, t) {
          this.m_cancel = E().CancelToken.source();
          const i = this.BGetUploadsAreInSerial() ? 1 : 4;
          let s;
          const n = this.UploadSingleImage.bind(this);
          return (
            (s =
              i > 1
                ? await Ye(
                    this.GetUploadImages(),
                    i,
                    n,
                    e,
                    t,
                    this.m_cancel.token,
                  )
                : await (async function (e, t, i, s, n) {
                    const a = Fe(e, i, s),
                      o = [];
                    for (const e of a) {
                      e.status = "uploading";
                      const i = await t(e, e.filename, e.language ?? -1, n);
                      (e.status = i.bSuccess ? "success" : "failed"),
                        (e.message =
                          !i.bSuccess && i.elErrorMessage
                            ? i.elErrorMessage
                            : ""),
                        o.push({
                          bSuccess: i.bSuccess,
                          image: e,
                          uploadResult: i.result,
                        });
                    }
                    return o;
                  })(this.GetUploadImages(), n, e, t, this.m_cancel.token)),
            s
          );
        }
        CancelAllUploads() {
          this.m_cancel?.cancel((0, p.we)("#ImageUpload_CancelRequest"));
        }
      }
      function Ve(e, t, i) {
        const s = me(t.width, i),
          n = me(t.height, i);
        return {
          sKey: `${e}_${s}x${n}`,
          width: s,
          height: n,
          bEnforceDimensions: !t.bDisableEnforceDimensions,
          artworkType: e,
          bHiddenFromDropdown: "hero" === e,
          bDeprecated: (i ?? 0) >= 1,
          fnGetLabelText() {
            return "spotlight" == this.artworkType
              ? (0, p.we)("#EventEditor_ArtworkType_store_spotlight")
              : (0, p.we)("#EventEditor_ArtworkType_" + this.artworkType);
          },
        };
      }
      class Ze extends Qe {
        m_filesToUpload = S.sH.array();
        m_filesCompleted = S.sH.array();
        m_clanImagesV2;
        m_clanSteamID;
        m_rgImageOptions;
        m_localizedImageGroupPrimaryImage;
        m_lastError = void 0;
        constructor(e, t, i, s) {
          var n;
          super(),
            (0, S.Gn)(this),
            (this.m_clanSteamID = e),
            (this.m_rgImageOptions =
              ((n = t),
              n?.flatMap((e) => {
                const t = Te[e];
                if (
                  ((0, z.wT)(Boolean(t), `Artwork Type not in Map ${e}`),
                  "number" == typeof t.width && "number" == typeof t.height)
                )
                  return [Ve(e, t)];
                {
                  let i = Math.max(
                    Array.isArray(t.width) ? t.width.length : 1,
                    Array.isArray(t.height) ? t.height.length : 1,
                  );
                  return Array.from({ length: i }, (i, s) => Ve(e, t, s));
                }
              }))),
            (this.m_localizedImageGroupPrimaryImage = i),
            (this.m_clanImagesV2 = s ?? !1);
        }
        GetClanSteamID() {
          return this.m_clanSteamID;
        }
        async AddImage(e, t = 0) {
          const { language: i } = (function (e, t = 0) {
            let i = e.lastIndexOf(".");
            -1 != i && (e = e.slice(0, i).toLowerCase());
            let s = null,
              n = 0;
            e.endsWith("korean") && ((s = 4), (n = 6));
            for (let t = 0; t < 31; ++t) {
              const i = (0, o.ww)(t);
              if (i.length <= n) continue;
              if (e.endsWith(i) && e.length > i.length + 2) {
                const a = e[e.length - i.length - 1];
                /\p{Alphabetic}|\p{Number}/u.test(a) ||
                  ((s = t), (n = i.length));
              }
              const a = (0, o.Lg)(t);
              a.length <= n || (e.endsWith(a) && ((s = t), (n = a.length)));
            }
            return {
              language: s ?? t,
              baseFilename:
                n > 0
                  ? ((a = e.substring(0, e.length - n)),
                    a.replace(/[\s_-]+$/g, ""))
                  : e,
            };
            var a;
          })(e.name, t);
          return this.AddImageForLanguage(e, i);
        }
        async AddImageForLanguage(e, t) {
          if (!((0, Be.aL)(e.type) || (d.iA.is_support && (0, Be.Uz)(e.type))))
            return !1;
          const i = await (0, Be.zB)(e, (0, Be.Uz)(e.type));
          if (!i) return !1;
          const s = new Ge(
            e,
            e.name,
            t,
            this.m_rgImageOptions,
            i.src,
            (0, Be.II)(i),
            i,
            this.m_localizedImageGroupPrimaryImage,
          );
          return (this.m_filesToUpload = [...this.m_filesToUpload, s]), !0;
        }
        async AddExistingClanImage(e, t = 0) {
          const i = G.z.GetHashAndExt(e);
          if (!i) return !1;
          const s = G.z.GenerateEditableURLFromHashAndExt(
              this.m_clanSteamID,
              i,
            ),
            n = await E()({ url: s, method: "GET", responseType: "blob" }),
            a = (0, ae.pE)(n.data, e.file_name);
          return await this.AddImage(a, t);
        }
        DeleteUploadImageByIndex(e) {
          this.m_filesToUpload.splice(e, 1),
            (this.m_filesToUpload = [...this.m_filesToUpload]);
        }
        DeleteUploadImage(e) {
          let t = this.m_filesToUpload.findIndex(
            (t) => e.file == t.file && e.uploadTime == t.uploadTime,
          );
          t >= 0 && this.DeleteUploadImageByIndex(t);
        }
        ClearImages() {
          this.m_filesToUpload = S.sH.array();
        }
        GetFilesUploaded() {
          return this.m_filesCompleted;
        }
        GetLastErrorFile() {
          return this.m_lastError;
        }
        GetCompletedFiles() {
          return this.m_filesCompleted.length;
        }
        GetTotalFiles() {
          return this.m_filesToUpload.length;
        }
        GetFilesToUpload() {
          return this.m_filesToUpload.map((e) => e.file);
        }
        GetUploadImages() {
          return this.m_filesToUpload;
        }
        BHasError() {
          return null != this.m_lastError;
        }
        BAllDone() {
          return (
            this.m_filesCompleted.length > 0 &&
            this.m_filesCompleted.length == this.m_filesToUpload.length
          );
        }
        BIsFileCompleted(e) {
          return -1 != this.m_filesCompleted.indexOf(e);
        }
        RetryAllFailedUploads() {
          this.CancelAllUploads(), this.UploadAllImages();
        }
        async handleUploadRefresh(e) {
          await Q.LoadClanImages(this.m_clanSteamID, !0, e);
        }
        BGetUploadsAreInSerial() {
          return !1;
        }
        async UploadSingleImage(e, t, i, s) {
          const n = e.file,
            a = e.GetCurrentImageOption(),
            o = e.GetResizeDimension(),
            r = new FormData();
          r.append("clanimage", n, t),
            r.append("sessionid", d.TS.SESSIONID),
            this.m_clanImagesV2 && r.append("clan_images_v2", "1"),
            a?.artworkType && r.append("arttype", a.artworkType),
            o &&
              o.length > 0 &&
              r.append(
                "resize",
                o.map((e) => e.width + "x" + e.height).join(","),
              );
          let l = "/uploadimage/";
          const c = this.m_localizedImageGroupPrimaryImage;
          c &&
            ((l = "/ajaxuploadlocalizedimage/"),
            r.append("origimagehash", c.image_hash),
            c.thumbnail_hash && r.append("thumbhash", c.thumbnail_hash),
            r.append("extension", "" + c.file_type),
            r.append("language", "" + i));
          const h = t.split(".").pop()?.toLocaleLowerCase();
          ("webm" != h && "mp4" != h) ||
            (r.append("video_width", "" + e.width),
            r.append("video_height", "" + e.height));
          let p,
            g =
              d.TS.COMMUNITY_BASE_URL +
              "/gid/" +
              this.m_clanSteamID.ConvertTo64BitString() +
              l,
            u = {
              cancelToken: s,
              withCredentials: !0,
              headers: { "Content-Type": "multipart/form-data" },
            },
            m = !0;
          try {
            (p = await E().post(g, r, u)), this.m_filesCompleted.push(n);
          } catch (e) {
            (m = !1),
              (this.m_lastError = {
                file: n,
                status: e.response ? e.response.status : 500,
                message: (0, Oe.H)(e).strErrorMsg,
              }),
              (p = e.response);
          }
          return (
            c || (await this.handleUploadRefresh(s)),
            { bSuccess: m, result: p.data }
          );
        }
      }
      (0, s.Cg)([S.sH], Ze.prototype, "m_filesToUpload", void 0),
        (0, s.Cg)([S.sH], Ze.prototype, "m_filesCompleted", void 0),
        (0, s.Cg)([S.sH], Ze.prototype, "m_lastError", void 0),
        (0, s.Cg)([D.o], Ze.prototype, "AddImage", null),
        (0, s.Cg)([D.o], Ze.prototype, "AddExistingClanImage", null),
        (0, s.Cg)([D.o], Ze.prototype, "DeleteUploadImageByIndex", null),
        (0, s.Cg)([D.o], Ze.prototype, "DeleteUploadImage", null),
        (0, s.Cg)([D.o], Ze.prototype, "ClearImages", null);
      class We extends Ze {
        constructor(e, t, i) {
          super(e, t, i, !1);
        }
      }
      const Je = (0, x.PA)((e) => {
        const {
            clanSteamID: t,
            inputClanImage: i,
            nWidth: s,
            nHeight: o,
            setImage: r,
          } = e,
          l = a.useMemo(() => ({ width: s, height: o }), [s, o]),
          [c, h] = a.useState(void 0),
          [g, u] = a.useState(Boolean(i)),
          [m, I] = a.useState(!1),
          C = (function (e, t, i) {
            const s = (0, ke.wm)(t instanceof Array ? t : [t]),
              n = e.ConvertTo64BitString();
            return a.useMemo(() => new We(e, s, i), [n, s]);
          })(t, "dummy"),
          L = a.useCallback(
            async (e) => {
              if (
                (C.ClearImages(),
                e && (u(!0), await C.AddExistingClanImage(e, 0)))
              ) {
                h(e);
                const t = C.GetUploadImages()[0].IsValidAssetType(l);
                0 != t.error.length ||
                  t.needsCrop ||
                  (i && i.image_hash == e.image_hash) ||
                  r(e);
              }
              u(!1);
            },
            [C, i, r, l],
          );
        a.useEffect(() => {
          L(i);
        }, [L, i]);
        let T,
          w = "",
          A = !1;
        if (C && C.GetFilesToUpload().length > 0) {
          T = C.GetUploadImages()[0];
          const e = T.IsValidAssetType(l);
          (w = e.error), (A = e.needsCrop);
        }
        return (0, n.jsxs)(n.Fragment, {
          children: [
            g
              ? (0, n.jsx)(he.t, {
                  size: "medium",
                  string: (0, p.we)("#Loading"),
                })
              : Boolean(c) &&
                (0, n.jsx)("div", {
                  className: Ne.Image,
                  style: {
                    backgroundImage: `url( '${T ? T.dataUrl : c.url}' )`,
                    height: `${o}px`,
                    width: `${s}px`,
                  },
                }),
            Boolean(w) && (0, n.jsx)("p", { children: w }),
            A &&
              (0, n.jsx)(ee.$n, {
                onClick: (e) => {
                  const t = (0, ae.uX)(e);
                  let i = C.GetUploadImages()[0];
                  (0, ie.pg)(
                    (0, n.jsx)(ye, {
                      ownerWin: t,
                      uploadFile: i,
                      forceResolution: { width: s, height: o },
                      fileType: 3,
                    }),
                    t,
                  );
                },
                children: (0, p.we)("#BBCode_ResizeImage"),
              }),
            Boolean(T && T.bCropped) &&
              (0, n.jsxs)(n.Fragment, {
                children: [
                  (0, n.jsx)("div", {
                    children: (0, p.we)(
                      "#ClanImagePickAndResize_UploadStatus",
                      T.status,
                    ),
                  }),
                  m
                    ? (0, n.jsx)(he.t, {
                        string: (0, p.we)("#Uploading"),
                        size: "small",
                      })
                    : (0, n.jsx)(ee.$n, {
                        onClick: async () => {
                          I(!0);
                          try {
                            const e = await C.UploadAllImages(l),
                              i = Object.values(e);
                            if (
                              i &&
                              i.length > 0 &&
                              ((0, z.wT)(
                                1 == i.length,
                                "ClanImagePickForCertainSize expected size 1, got " +
                                  i.length,
                              ),
                              i[0].bSuccess)
                            ) {
                              const e = i[0].uploadResult,
                                s = G.z.GenerateURLFromHashAndExt(
                                  t,
                                  G.z.GetHashAndExt(e),
                                ),
                                n = G.z.GenerateURLFromHashAndExt(
                                  t,
                                  G.z.GetThumbHashAndExt(e),
                                ),
                                a = {
                                  imageid: -11231412,
                                  image_hash: e.image_hash,
                                  thumbnail_hash: e.thumbnail_hash,
                                  file_type: e.file_type,
                                  file_name: e.file_name,
                                  clanAccountID: t.GetAccountID(),
                                  url: s,
                                  thumb_url: n,
                                  uploaded_time: Date.now() / 1e3,
                                };
                              h(a), r(a);
                            }
                          } finally {
                            I(!1);
                          }
                        },
                        children: (0, p.we)(
                          "#ClanImagePickAndResize_UploadImage",
                        ),
                      }),
                ],
              }),
            (0, n.jsx)(ee.$n, {
              onClick: (e) => {
                const t = new B.b(d.UF.CLANSTEAMID);
                (0, ie.pg)(
                  (0, n.jsx)(je, {
                    clanSteamID: t,
                    fnImageSelectCallBack: (e) => L(e),
                  }),
                  (0, ae.uX)(e),
                );
              },
              children: (0, p.we)("#BBCode_ChooseImage", s, o),
            }),
          ],
        });
      });
      var Xe;
      let Ke = class extends a.Component {
        static {
          Xe = this;
        }
        m_cancelSignal = E().CancelToken.source();
        static s_formattingHelp = new Map();
        constructor(e) {
          super(e),
            (this.state = {
              formattingHelp: {
                __html: Xe.s_formattingHelp.has(e.formatType)
                  ? Xe.s_formattingHelp.get(e.formatType)
                  : "",
              },
            });
        }
        componentDidMount() {
          this.AjaxGetFormattingHelp().catch((e) => {
            this.setState((0, Oe.H)(e));
          });
        }
        componentWillUnmount() {
          this.m_cancelSignal.cancel(
            "FormattingHelpWidget component unmounted",
          );
        }
        static GetHelpURL(e, t) {
          return (
            d.TS.COMMUNITY_BASE_URL +
            "comment/" +
            e +
            "/formattinghelp" +
            (t ? "?ajax=1" : "")
          );
        }
        async AjaxGetFormattingHelp() {
          if ("" == this.state.formattingHelp.__html) {
            let e,
              t = { sessionid: d.TS.SESSIONID };
            (e = await E().get(Xe.GetHelpURL(this.props.formatType, !0), {
              params: t,
              cancelToken: this.m_cancelSignal.token,
            })),
              Xe.s_formattingHelp.set(this.props.formatType, e.data),
              this.setState({ formattingHelp: { __html: e.data } });
          }
        }
        render() {
          return this.state.strErrorMsg
            ? (0, n.jsxs)("div", {
                children: [
                  this.state.strErrorMsg,
                  (0, n.jsx)("br", {}),
                  this.state.errorCode,
                ],
              })
            : "" == this.state.formattingHelp.__html
              ? (0, n.jsx)(he.t, {})
              : (0, n.jsx)(C.o0, {
                  strTitle: (0, p.we)(
                    "#EventEditor_FormattingHelp_GetHelpLink",
                  ),
                  strDescription: "",
                  closeModal: this.props.closeModal,
                  onOK: this.props.closeModal,
                  onCancel: this.props.closeModal,
                  bAlertDialog: !0,
                  className: "ModernBBStyles",
                  children: (0, n.jsx)("div", {
                    dangerouslySetInnerHTML: this.state.formattingHelp,
                  }),
                });
        }
      };
      Ke = Xe = (0, s.Cg)([x.PA], Ke);
      var qe,
        $e = i(24546),
        et = i(34091),
        tt = i(27793);
      let it = class extends a.Component {
        descAutoTextAreaRef = a.createRef();
        constructor(e) {
          super(e), (this.state = { bShowDragTarget: !1 });
        }
        componentDidMount() {
          Q.AddClanImageDragListener(this.ClanImageDragListener);
        }
        componentWillUnmount() {
          Q.RemoveClanImageDragListener(this.ClanImageDragListener);
        }
        ClanImageDragListener(e, t) {
          this.state.bShowDragTarget != t &&
            this.setState({ bShowDragTarget: t });
        }
        onFocus(e) {
          e && e.target.select();
        }
        InsertText(e) {
          st.replaceSelection(this.GetTextAreaRef().current, e);
        }
        OnTextAreaDropListener(e) {
          if (
            (e.preventDefault(),
            e.stopPropagation(),
            e.dataTransfer.items && e.dataTransfer.items[0])
          ) {
            let t = e.dataTransfer.getData("text");
            if (t && t.length > 0)
              for (let e of [G.z.GetBaseURL(), G.z.GetBaseURLV2()])
                if (t.startsWith(e)) {
                  let i = "[img]" + $e.lw + "/" + t.substr(e.length) + "[/img]";
                  st.replaceSelection(this.GetTextAreaRef().current, i);
                  break;
                }
          }
        }
        GetTextAreaRef() {
          return this.descAutoTextAreaRef && this.descAutoTextAreaRef.current
            ? this.descAutoTextAreaRef.current.GetTextAreaRef()
            : void 0;
        }
        render() {
          return (0, n.jsxs)(a.Fragment, {
            children: [
              (0, n.jsx)(nt, {
                pathToImages:
                  d.TS.COMMUNITY_CDN_URL + "public/images/sharedfiles/guides/",
                fnTextareaRef: this.GetTextAreaRef,
                emoticonStore: this.props.emoticonStore,
                supportBBCodes: this.props.limitBBCode
                  ? this.props.limitBBCode
                  : N.Kl,
                bSupportHTMLImport: this.props.bSupportHTMLImport,
                showFormatHelp: this.props.showFormatHelp,
                bEmbeddedInDialog: this.props.bEmbeddedInDialog,
                clanSteamID: this.props.clanSteamID,
              }),
              (0, n.jsx)("div", {
                className: (0, h.A)(
                  ge().DescriptionCtn,
                  ge().BBCodeEditorInputStyles,
                  this.state.bShowDragTarget ? ge().DragTarget : "",
                  this.props.className ? this.props.className : "",
                ),
                children: (0, n.jsx)($, {
                  cols: 90,
                  rows: this.props.nOverridesRows || 22,
                  maxLength: 64e3,
                  className: (0, h.A)(
                    ge().DefaultEditor,
                    this.props.classNameForTextArea
                      ? this.props.classNameForTextArea
                      : "",
                  ),
                  placeholder: this.props.strPlaceholder,
                  ref: this.descAutoTextAreaRef,
                  value: this.props.fnGetCurText(),
                  onChange: this.props.fnOnTextChange,
                  onDrop: this.OnTextAreaDropListener,
                  emoticonStore: this.props.emoticonStore,
                  fnSetText: this.props.fnSetText,
                  supportBBCodes: this.props.limitBBCode
                    ? this.props.limitBBCode
                    : N.Kl,
                }),
              }),
            ],
          });
        }
      };
      (0, s.Cg)([g.oI], it.prototype, "ClanImageDragListener", null),
        (0, s.Cg)([g.oI], it.prototype, "onFocus", null),
        (0, s.Cg)([g.oI], it.prototype, "OnTextAreaDropListener", null),
        (0, s.Cg)([g.oI], it.prototype, "GetTextAreaRef", null),
        (it = (0, s.Cg)([x.PA], it));
      class st {
        static BIsFireFox() {
          return Boolean(
            new RegExp(/Firefox\/([0-9\.]+)(?:\s|$)/i).exec(
              navigator.userAgent,
            ),
          );
        }
        static replaceSelection(e, t) {
          let i = e.selectionStart;
          e.focus(),
            st.InsertTextAtSelect(t, e),
            e.setSelectionRange(i, i + t.length);
        }
        static getSelectedString(e) {
          return e.value.substr(
            e.selectionStart,
            e.selectionEnd - e.selectionStart,
          );
        }
        static wrapBBCode(e, t, i) {
          let s = st.getSelectedString(i),
            n = "";
          (n =
            0 == s.indexOf(e) && s.lastIndexOf(t) == s.length - t.length
              ? s.substr(e.length, s.length - e.length - t.length)
              : e + s + t),
            st.replaceSelection(i, n);
        }
        static append(e, t) {
          t &&
            (t.focus(),
            t.setSelectionRange(t.value.length, t.value.length),
            st.InsertTextAtSelect(e, t));
        }
        static ClearTextArea(e) {
          if (e) {
            e.focus();
            const t = 0,
              i = e.value.length;
            if (t !== i) {
              e.setRangeText
                ? e.setRangeText("", t, i, "preserve")
                : (e.value = "");
              const s = new Event("input", { bubbles: !0 });
              e.dispatchEvent(s);
            }
            e.focus();
          }
        }
        static overwrite(e, t) {
          t && (st.ClearTextArea(t), st.InsertTextAtSelect(e, t));
        }
        static InsertTextAtSelect(e, t) {
          const i = t.selectionStart,
            s = t.selectionEnd;
          if (null !== i && null !== s) {
            t.setRangeText
              ? t.setRangeText(e, i, s, "preserve")
              : (t.value = t.value.slice(0, i) + e + t.value.slice(s));
            const n = new Event("input", { bubbles: !0 });
            t.dispatchEvent(n),
              (t.selectionStart = t.selectionEnd = i + e.length);
          }
          t.focus();
        }
      }
      let nt = class extends a.Component {
        m_linkPopupRef = a.createRef();
        onBold() {
          st.wrapBBCode("[b]", "[/b]", this.props.fnTextareaRef().current);
        }
        onItalics() {
          st.wrapBBCode("[i]", "[/i]", this.props.fnTextareaRef().current);
        }
        onUnderline() {
          st.wrapBBCode("[u]", "[/u]", this.props.fnTextareaRef().current);
        }
        onStrikeThrough() {
          st.wrapBBCode(
            "[strike]",
            "[/strike]",
            this.props.fnTextareaRef().current,
          );
        }
        onHeader() {
          st.wrapBBCode("[h1]", "[/h1]", this.props.fnTextareaRef().current);
        }
        onHeader2() {
          st.wrapBBCode("[h2]", "[/h2]", this.props.fnTextareaRef().current);
        }
        onHeader3() {
          st.wrapBBCode("[h3]", "[/h3]", this.props.fnTextareaRef().current);
        }
        onUnorderedList() {
          this.handleList("list");
        }
        onOrderedList() {
          this.handleList("olist");
        }
        handleList(e) {
          let t = this.props.fnTextareaRef().current,
            i = "[" + e + "]\n",
            s = "[/" + e + "]";
          if (t.selectionStart == t.selectionEnd)
            st.wrapBBCode(i + "[*]", "\n" + s, t);
          else {
            let e =
              i +
              st
                .getSelectedString(t)
                .split("\n")
                .map((e) => (e.match(/\*+\s/) ? "[*]" : "[*] ") + e)
                .join("\n") +
              "\n" +
              s;
            st.replaceSelection(t, e);
          }
        }
        OnAddLink(e) {
          (0, ie.HT)(
            (0, n.jsx)(rt, { textareaRef: this.props.fnTextareaRef() }),
            (0, ae.uX)(e),
          );
        }
        ShowHelpDialog(e) {
          this.props.showFormatHelp &&
            (0, ie.HT)(
              (0, n.jsx)(Ke, { formatType: this.props.showFormatHelp }),
              (0, ae.uX)(e),
            );
        }
        OnConvertHTMLToBBCodeDialog(e) {
          const t = (0, ae.uX)(e);
          (0, ie.HT)(
            (0, n.jsx)(gt, {
              ownerWindow: t,
              textareaRef: this.props.fnTextareaRef(),
            }),
            t,
          );
        }
        OnOpenYoutubeDialog(e) {
          let t = d.TS.IMG_URL + "applications/community/";
          (0, ie.HT)(
            (0, n.jsx)(ot, {
              textareaRef: this.props.fnTextareaRef(),
              pathToImages: t,
            }),
            (0, ae.uX)(e),
          );
        }
        OnOpenImageDialog(e) {
          (0, ie.HT)(
            (0, n.jsx)(lt, { textareaRef: this.props.fnTextareaRef() }),
            (0, ae.uX)(e),
          );
        }
        OnOpenSpeakerDialog(e) {
          (0, ie.pg)(
            (0, n.jsx)(ct, {
              clanSteamID: this.props.clanSteamID,
              textareaRef: this.props.fnTextareaRef(),
            }),
            (0, ae.uX)(e),
          );
        }
        OnEmoticonSelected(e, t = !1) {
          let i = `ː${e}ː`;
          st.replaceSelection(this.props.fnTextareaRef().current, i),
            this.props.fnTextareaRef().current.focus();
        }
        BSupports(e) {
          return this.props.supportBBCodes.findIndex((t) => t == e) >= 0;
        }
        render() {
          const {
            showFormatHelp: e,
            bEmbeddedInDialog: t,
            bSupportHTMLImport: i,
            pathToImages: s,
          } = this.props;
          let a;
          return (
            e &&
              (a = t
                ? (0, n.jsx)("span", {
                    className: (0, h.A)("ttip", ge().ActionGetHelp),
                    children: (0, n.jsx)(L.he, {
                      toolTipContent: (0, p.we)(
                        "#EventEditor_FormattingHelp_GetHelpLink",
                      ),
                      children: (0, n.jsxs)("a", {
                        href: Ke.GetHelpURL(this.props.showFormatHelp, !1),
                        target: d.TS.IN_CLIENT ? void 0 : "_blank",
                        children: [
                          (0, n.jsx)("img", { src: s + "/action_help.png" }),
                          " ",
                          (0, p.we)("#EventEditor_FormattingHelp_GetHelpLink"),
                        ],
                      }),
                    }),
                  })
                : (0, n.jsx)("span", {
                    onClick: this.ShowHelpDialog,
                    className: (0, h.A)("ttip", ge().ActionGetHelp),
                    children: (0, n.jsxs)(L.he, {
                      toolTipContent: (0, p.we)(
                        "#EventEditor_FormattingHelp_GetHelpLink",
                      ),
                      children: [
                        (0, n.jsx)("img", { src: s + "/action_help.png" }),
                        " ",
                        (0, p.we)("#EventEditor_FormattingHelp_GetHelpLink"),
                      ],
                    }),
                  })),
            (0, n.jsxs)("div", {
              className: ge().TextEditorToolBarContainer,
              children: [
                this.BSupports("b") &&
                  (0, n.jsx)(at, {
                    onClick: this.onBold,
                    tooltip: (0, p.we)("#Editor_Bold"),
                    imgURL: this.props.pathToImages + "/format_bold.png",
                  }),
                this.BSupports("u") &&
                  (0, n.jsx)(at, {
                    onClick: this.onUnderline,
                    tooltip: (0, p.we)("#Editor_Underline"),
                    imgURL: this.props.pathToImages + "/format_underline.png",
                  }),
                this.BSupports("i") &&
                  (0, n.jsx)(at, {
                    onClick: this.onItalics,
                    tooltip: (0, p.we)("#Editor_Italics"),
                    imgURL: this.props.pathToImages + "/format_italic.png",
                  }),
                this.BSupports("strike") &&
                  (0, n.jsx)(at, {
                    onClick: this.onStrikeThrough,
                    tooltip: (0, p.we)("#Editor_StrikeThrough"),
                    imgURL: this.props.pathToImages + "/format_strike.png",
                  }),
                Boolean(this.BSupports("url") && !t) &&
                  (0, n.jsx)(at, {
                    onClick: this.OnAddLink,
                    tooltip: (0, p.we)("#Editor_Link"),
                    imgURL: this.props.pathToImages + "/format_link.png",
                  }),
                this.BSupports("list") &&
                  (0, n.jsx)(at, {
                    onClick: this.onUnorderedList,
                    tooltip: (0, p.we)("#Editor_Unordered"),
                    imgURL: this.props.pathToImages + "/format_bullet.png",
                  }),
                this.BSupports("olist") &&
                  (0, n.jsx)(at, {
                    onClick: this.onOrderedList,
                    tooltip: (0, p.we)("#Editor_Ordered"),
                    imgURL: this.props.pathToImages + "/format_numbered.png",
                  }),
                this.BSupports("h1") &&
                  (0, n.jsx)(at, {
                    onClick: this.onHeader,
                    tooltip: (0, p.we)("#Editor_Header"),
                    imgURL: this.props.pathToImages + "/format_header1.png",
                  }),
                this.BSupports("h2") &&
                  (0, n.jsx)(at, {
                    onClick: this.onHeader2,
                    tooltip: (0, p.we)("#Editor_Header2"),
                    imgURL: this.props.pathToImages + "/format_header2.png",
                  }),
                this.BSupports("h3") &&
                  (0, n.jsx)(at, {
                    onClick: this.onHeader3,
                    tooltip: (0, p.we)("#Editor_Header3"),
                    imgURL: this.props.pathToImages + "/format_header3.png",
                  }),
                this.BSupports("previewyoutube") &&
                  (0, n.jsx)(at, {
                    onClick: this.OnOpenYoutubeDialog,
                    tooltip: (0, p.we)("#EventEditor_InsertYouTube"),
                    imgURL:
                      "data:image/png;base64,",
                  }),
                (0, n.jsx)("span", {
                  className: "ttip",
                  children:
                    this.props.emoticonStore &&
                    (0, n.jsx)(L.he, {
                      toolTipContent: (0, p.we)("#Editor_Emoticon"),
                      children: (0, n.jsx)(te.A, {
                        title: " ",
                        className: (0, h.A)(ge().EmoteOuter),
                        disabled: !1,
                        OnEmoticonSelected: this.OnEmoticonSelected,
                        rtLastAckedNewEmoticons: Number.MAX_VALUE,
                        emoticonStore: this.props.emoticonStore,
                        useImg: this.props.pathToImages + "/format_emote.png",
                        contextOptions: {
                          bOverlapHorizontal: !0,
                          bDisablePopTop: !0,
                        },
                      }),
                    }),
                }),
                Boolean(this.BSupports("img") && !t) &&
                  (0, n.jsx)(at, {
                    onClick: this.OnOpenImageDialog,
                    tooltip: (0, p.we)("#EventEditor_InsertImage"),
                    imgURL: this.props.pathToImages + "/insert_img.png",
                  }),
                Boolean(
                  d.iA.is_support &&
                    this.props.clanSteamID &&
                    this.BSupports("speaker"),
                ) &&
                  (0, n.jsx)(at, {
                    onClick: this.OnOpenSpeakerDialog,
                    tooltip: (0, p.we)("#EventEditor_AddSpeaker"),
                    imgURL: this.props.pathToImages + "/insert_img.png",
                  }),
                Boolean(i && !t) &&
                  (0, n.jsx)(at, {
                    onClick: this.OnConvertHTMLToBBCodeDialog,
                    className: ge().ActionImportHTML,
                    tooltip: (0, p.we)("#EventEditor_ImportFromHTML_ttip"),
                    children: (0, p.we)("#EventEditor_ImportHTML"),
                  }),
                a,
              ],
            })
          );
        }
      };
      function at(e) {
        return (0, n.jsx)("span", {
          onClick: e.onClick,
          className: e.className,
          children: (0, n.jsxs)(L.he, {
            toolTipContent: e.tooltip,
            className: "ttip",
            children: [
              Boolean(e.imgURL) && (0, n.jsx)("img", { src: e.imgURL }),
              e.children,
            ],
          }),
        });
      }
      (0, s.Cg)([g.oI], nt.prototype, "onBold", null),
        (0, s.Cg)([g.oI], nt.prototype, "onItalics", null),
        (0, s.Cg)([g.oI], nt.prototype, "onUnderline", null),
        (0, s.Cg)([g.oI], nt.prototype, "onStrikeThrough", null),
        (0, s.Cg)([g.oI], nt.prototype, "onHeader", null),
        (0, s.Cg)([g.oI], nt.prototype, "onHeader2", null),
        (0, s.Cg)([g.oI], nt.prototype, "onHeader3", null),
        (0, s.Cg)([g.oI], nt.prototype, "onUnorderedList", null),
        (0, s.Cg)([g.oI], nt.prototype, "onOrderedList", null),
        (0, s.Cg)([g.oI], nt.prototype, "OnAddLink", null),
        (0, s.Cg)([g.oI], nt.prototype, "ShowHelpDialog", null),
        (0, s.Cg)([g.oI], nt.prototype, "OnConvertHTMLToBBCodeDialog", null),
        (0, s.Cg)([g.oI], nt.prototype, "OnOpenYoutubeDialog", null),
        (0, s.Cg)([g.oI], nt.prototype, "OnOpenImageDialog", null),
        (0, s.Cg)([g.oI], nt.prototype, "OnOpenSpeakerDialog", null),
        (0, s.Cg)([g.oI], nt.prototype, "OnEmoticonSelected", null),
        (nt = (0, s.Cg)([x.PA], nt));
      let ot = class extends a.Component {
        state = { youtubeInput: "", alignment: et.V2.left };
        OnYoutubeInsertLink() {
          const e =
            this.state.youtubeInput && (0, P.XU)(this.state.youtubeInput);
          if (e) {
            if (this.state.alignment == et.V2.summary) {
              const t =
                "https://www.youtube.com/watch?v=" +
                e.strVideoID +
                (e.nStartSeconds ? "&t=" + e.nStartSeconds : "");
              st.wrapBBCode(t, "", this.props.textareaRef.current);
            } else {
              let t =
                "[previewyoutube=" +
                e.strVideoID +
                ";" +
                this.state.alignment +
                "]";
              st.wrapBBCode(
                t,
                "[/previewyoutube]",
                this.props.textareaRef.current,
              );
            }
            this.setState({ youtubeInput: "", alignment: et.V2.left });
          } else alert((0, p.we)("#EventEditor_InsertYouTube_NoURL"));
        }
        OnUrlChange(e) {
          this.state.youtubeInput != e.target.value &&
            this.setState({ youtubeInput: e.target.value });
        }
        OnLeftSelected() {
          this.setState({ alignment: et.V2.left });
        }
        OnRightSelected() {
          this.setState({ alignment: et.V2.right });
        }
        OnFullSelected() {
          this.setState({ alignment: et.V2.full });
        }
        OnSummarySelected() {
          this.setState({ alignment: et.V2.summary });
        }
        OnOuterDivClickPassDown(e) {}
        render() {
          return (0, n.jsx)(C.o0, {
            strTitle: (0, p.we)("#EventEditor_InsertYouTube"),
            strDescription: "",
            closeModal: this.props.closeModal,
            onCancel: this.props.closeModal,
            onOK: this.OnYoutubeInsertLink,
            strOKButtonText: (0, p.we)("#EventEditor_InsertYouTube"),
            className: ge().BBCodeEditorInputStyles,
            children: (0, n.jsxs)("div", {
              className: ge().YouTubeInput,
              children: [
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, p.we)("#EventEditor_InsertYouTube_URL"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          className: "DialogInput DialogTextInputBase",
                          ref: (e) => e && e.focus(),
                          type: "text",
                          value: this.state.youtubeInput,
                          onChange: this.OnUrlChange,
                          placeholder: (0, p.we)(
                            "#EventEditor_InsertYouTube_Placholder",
                          ),
                        }),
                      }),
                    ],
                  }),
                }),
                (0, n.jsxs)("div", {
                  className: "DialogInputLabelGroup",
                  children: [
                    (0, n.jsx)("div", {
                      className: "DialogLabel",
                      children: (0, p.we)(
                        "#EventEditor_InsertYouTube_Position",
                      ),
                    }),
                    (0, n.jsxs)("div", {
                      className: ge().YouTubePreviewInsertOption,
                      onClick: this.OnOuterDivClickPassDown,
                      children: [
                        (0, n.jsx)("input", {
                          type: "radio",
                          name: "YouTubePreviewInsertType",
                          id: et.V2.left,
                          value: et.V2.left,
                          checked: this.state.alignment == et.V2.left,
                          onChange: this.OnLeftSelected,
                        }),
                        (0, n.jsx)("label", {
                          htmlFor: et.V2.left,
                          children: (0, n.jsx)("span", {
                            children: (0, p.we)(
                              "#EventEditor_InsertYouTube_Left",
                            ),
                          }),
                        }),
                      ],
                    }),
                    (0, n.jsxs)("div", {
                      className: ge().YouTubePreviewInsertOption,
                      onClick: this.OnOuterDivClickPassDown,
                      children: [
                        (0, n.jsx)("input", {
                          type: "radio",
                          name: "YouTubePreviewInsertType",
                          id: et.V2.right,
                          value: et.V2.right,
                          checked: this.state.alignment == et.V2.right,
                          onChange: this.OnRightSelected,
                        }),
                        (0, n.jsx)("label", {
                          htmlFor: et.V2.right,
                          children: (0, n.jsx)("span", {
                            children: (0, p.we)(
                              "#EventEditor_InsertYouTube_Right",
                            ),
                          }),
                        }),
                      ],
                    }),
                    (0, n.jsxs)("div", {
                      className: ge().YouTubePreviewInsertOption,
                      onClick: this.OnOuterDivClickPassDown,
                      children: [
                        (0, n.jsx)("input", {
                          type: "radio",
                          name: "YouTubePreviewInsertType",
                          id: et.V2.full,
                          value: et.V2.full,
                          checked: this.state.alignment == et.V2.full,
                          onChange: this.OnFullSelected,
                        }),
                        (0, n.jsx)("label", {
                          htmlFor: et.V2.full,
                          children: (0, n.jsx)("span", {
                            children: (0, p.we)(
                              "#EventEditor_InsertYouTube_Full",
                            ),
                          }),
                        }),
                      ],
                    }),
                    (0, n.jsxs)("div", {
                      className: ge().YouTubePreviewInsertOption,
                      onClick: this.OnOuterDivClickPassDown,
                      children: [
                        (0, n.jsx)("input", {
                          type: "radio",
                          name: "YouTubePreviewInsertType",
                          id: et.V2.summary,
                          value: et.V2.summary,
                          checked: this.state.alignment == et.V2.summary,
                          onChange: this.OnSummarySelected,
                        }),
                        (0, n.jsx)("label", {
                          htmlFor: et.V2.summary,
                          children: (0, n.jsx)("span", {
                            children: (0, p.we)(
                              "#EventEditor_InsertYouTube_Summary",
                            ),
                          }),
                        }),
                      ],
                    }),
                  ],
                }),
              ],
            }),
          });
        }
      };
      (0, s.Cg)([g.oI], ot.prototype, "OnYoutubeInsertLink", null),
        (0, s.Cg)([g.oI], ot.prototype, "OnUrlChange", null),
        (0, s.Cg)([g.oI], ot.prototype, "OnLeftSelected", null),
        (0, s.Cg)([g.oI], ot.prototype, "OnRightSelected", null),
        (0, s.Cg)([g.oI], ot.prototype, "OnFullSelected", null),
        (0, s.Cg)([g.oI], ot.prototype, "OnSummarySelected", null),
        (0, s.Cg)([g.oI], ot.prototype, "OnOuterDivClickPassDown", null),
        (ot = (0, s.Cg)([x.PA], ot));
      let rt = class extends a.Component {
        static {
          qe = this;
        }
        state = { textToDisplay: "", strURL: "" };
        static m_regExp = new RegExp(/\[url=([^\]]*)\]([^\[\]]+)\[\/url\]/i);
        LoadFromTextArea() {
          const { textareaRef: e } = this.props;
          if (e && e.current) {
            let t = st.getSelectedString(e.current),
              i = qe.m_regExp.exec(t);
            i
              ? this.setState({ strURL: i[1], textToDisplay: i[2] })
              : this.setState({ textToDisplay: t });
          }
        }
        componentDidMount() {
          this.LoadFromTextArea();
        }
        onLinkTitleUpdate(e) {
          this.setState({ textToDisplay: e.target.value });
        }
        onLinkURLUpdate(e) {
          this.setState({ strURL: e.target.value });
        }
        onInsertLink() {
          const { strURL: e, textToDisplay: t } = this.state;
          let i = "[url=" + e + "]" + t + "[/url]";
          st.replaceSelection(this.props.textareaRef.current, i);
        }
        render() {
          return (0, n.jsx)(C.o0, {
            strTitle: (0, p.we)("#Editor_Link"),
            strDescription: "",
            closeModal: this.props.closeModal,
            onOK: this.onInsertLink,
            onCancel: this.props.closeModal,
            strOKButtonText: (0, p.we)("#EventEditor_InsertLinkURL"),
            className: ge().BBCodeEditorInputStyles,
            children: (0, n.jsxs)("div", {
              className: ge().EventEditorLinkInput,
              children: [
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, p.we)("#EventEditor_LinkDescription"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          type: "text",
                          onChange: this.onLinkTitleUpdate,
                          value: this.state.textToDisplay,
                          className: "DialogInput DialogTextInputBase",
                        }),
                      }),
                    ],
                  }),
                }),
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, p.we)("#EventEditor_LinkURL"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          type: "text",
                          onChange: this.onLinkURLUpdate,
                          value: this.state.strURL,
                          className: "DialogInput DialogTextInputBase",
                        }),
                      }),
                    ],
                  }),
                }),
              ],
            }),
          });
        }
      };
      (0, s.Cg)([g.oI], rt.prototype, "onLinkTitleUpdate", null),
        (0, s.Cg)([g.oI], rt.prototype, "onLinkURLUpdate", null),
        (0, s.Cg)([g.oI], rt.prototype, "onInsertLink", null),
        (rt = qe = (0, s.Cg)([x.PA], rt));
      let lt = class extends a.Component {
        refFirstInput;
        state = { imgURL: "", anchorURL: "" };
        componentDidMount() {
          this.refFirstInput && this.refFirstInput.current.focus();
        }
        OnImageInsert() {
          const { anchorURL: e, imgURL: t } = this.state;
          let i = "",
            s = "";
          e && e.length > 0 && ((i += "[url=" + e + "]"), (s = "[/url]" + s)),
            (i += "[img]" + t),
            (s = "[/img]" + s),
            st.wrapBBCode(i, s, this.props.textareaRef.current);
        }
        OnImageURLChange(e) {
          this.state.imgURL != e.target.value &&
            this.setState({ imgURL: e.target.value });
        }
        OnAnchorURLChange(e) {
          this.state.anchorURL != e.target.value &&
            this.setState({ anchorURL: e.target.value });
        }
        render() {
          const { imgURL: e, anchorURL: t } = this.state;
          return (0, n.jsx)(C.o0, {
            strTitle: (0, p.we)("#EventEditor_InsertImage_Title"),
            strDescription: "",
            closeModal: this.props.closeModal,
            onCancel: this.props.closeModal,
            onOK: this.OnImageInsert,
            strOKButtonText: (0, p.we)("#EventEditor_InsertImage_Title"),
            className: ge().BBCodeEditorInputStyles,
            children: (0, n.jsxs)("div", {
              className: ge().EventEditorLinkInput,
              children: [
                (0, n.jsx)("p", {
                  children: (0, p.we)("#EventEditor_InsertImage_Desc"),
                }),
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, p.we)("#EventEditor_InsertImage_URL"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          className: "DialogInput DialogTextInputBase",
                          type: "text",
                          value: e,
                          onChange: this.OnImageURLChange,
                          placeholder: (0, p.we)(
                            "#EventEditor_InsertImage_Placeholder",
                          ),
                          ref: this.refFirstInput,
                        }),
                      }),
                    ],
                  }),
                }),
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, p.we)("#EventEditor_InsertImage_Anchor"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          className: "DialogInput DialogTextInputBase",
                          type: "text",
                          value: t,
                          onChange: this.OnAnchorURLChange,
                          placeholder: (0, p.we)(
                            "#EventEditor_InsertImage_Placeholder",
                          ),
                        }),
                      }),
                    ],
                  }),
                }),
              ],
            }),
          });
        }
      };
      (0, s.Cg)([g.oI], lt.prototype, "OnImageInsert", null),
        (0, s.Cg)([g.oI], lt.prototype, "OnImageURLChange", null),
        (0, s.Cg)([g.oI], lt.prototype, "OnAnchorURLChange", null),
        (lt = (0, s.Cg)([x.PA], lt));
      const ct = (e) => {
        const [t, i] = a.useState(""),
          [s, o] = a.useState(""),
          [r, l] = a.useState(""),
          [c, h] = a.useState(""),
          [g, u] = a.useState(void 0),
          [m, I] = a.useState(void 0),
          { data: T } = (0, oe.Dv)(),
          { isLoading: w, data: A } = (0, re.js)(m?.GetAccountID()),
          x = 0 != t.trim().length && 0 != c.trim().length;
        return (0, n.jsx)(C.o0, {
          strTitle: (0, p.we)("#EventEditor_AddSpeaker"),
          strDescription: (0, p.we)("#EventEditor_AddSpeaker_Desc"),
          closeModal: e.closeModal,
          onCancel: e.closeModal,
          bOKDisabled: !x,
          onOK: () => {
            let i = `[speaker name="${t.trim()}"`;
            if (
              (s.trim().length > 0 && (i += ` title="${s}"`),
              r.trim().length > 0 && (i += ` company="${r}"`),
              g)
            ) {
              i += ` photo="${$e.lw + "/" + g.clanAccountID + "/" + G.z.GetHashAndExt(g)}"`;
            }
            m && (i += ` steamid="${m.ConvertTo64BitString()}"`),
              (i += `]${c}[/speaker]`),
              st.replaceSelection(e.textareaRef.current, i);
          },
          className: ge().BBCodeEditorInputStyles,
          children: (0, n.jsxs)("div", {
            className: ge().InsertSpeakerCtn,
            children: [
              (0, n.jsx)(ee.pd, {
                type: "text",
                label: (0, p.we)("#EventEditor_AddSpeaker_Name"),
                value: t,
                onChange: (e) => i(e.target.value),
                focusOnMount: !0,
              }),
              (0, n.jsxs)("div", {
                className: ge().TitleGroup,
                children: [
                  (0, n.jsx)(ee.pd, {
                    type: "text",
                    label: (0, p.we)("#EventEditor_AddSpeaker_Title"),
                    value: s,
                    onChange: (e) => o(e.target.value),
                  }),
                  (0, n.jsx)(ee.pd, {
                    type: "text",
                    label: (0, p.we)("#EventEditor_AddSpeaker_Company"),
                    value: r,
                    onChange: (e) => l(e.target.value),
                  }),
                ],
              }),
              (0, n.jsx)(L.he, {
                toolTipContent: (0, p.we)(
                  "#EventEditor_AssociateSteamAccount_ttip",
                ),
                children: (0, n.jsxs)("div", {
                  className: "DialogLabel",
                  children: [
                    (0, p.we)("#EventEditor_AssociateSteamAccount"),
                    " (?)",
                  ],
                }),
              }),
              (0, n.jsxs)("div", {
                children: [
                  w &&
                    (0, n.jsx)(he.t, {
                      string: (0, p.we)("#Loading"),
                      size: "small",
                    }),
                  Boolean(A) &&
                    (0, n.jsxs)("a", {
                      href:
                        d.TS.COMMUNITY_BASE_URL +
                        "profiles/" +
                        m.ConvertTo64BitString(),
                      target: "_blank",
                      children: [
                        A
                          ? (0, n.jsx)("img", {
                              style: { marginRight: "8px" },
                              src: A.avatar_url,
                            })
                          : null,
                        A ? A.m_strPlayerName : null,
                      ],
                    }),
                  (0, n.jsxs)("div", {
                    className: ge().AssociateRowCtn,
                    children: [
                      (0, n.jsx)(ee.$n, {
                        onClick: () => I(new B.b(d.iA.steamid)),
                        children: (0, p.we)("#EventEditor_SteamAccount_addme"),
                      }),
                      (0, n.jsx)(ee.$n, {
                        onClick: (e) =>
                          (0, U.lX)(
                            (0, n.jsx)(ht, { friends: T, setSteamID: I }),
                            e,
                          ),
                        children: (0, p.we)(
                          "#EventEditor_SteamAccount_addfriend",
                        ),
                      }),
                      (0, n.jsx)(ee.$n, {
                        onClick: () => I(void 0),
                        children: (0, p.we)("#EventEditor_SteamAccount_clear"),
                      }),
                    ],
                  }),
                ],
              }),
              (0, n.jsxs)("div", {
                className: ge().PhotoCtn,
                children: [
                  (0, n.jsx)("div", {
                    className: "DialogLabel",
                    children: (0, p.we)("#EventEditor_ChoosePhoto"),
                  }),
                  (0, n.jsx)(Je, {
                    clanSteamID: e.clanSteamID,
                    inputClanImage: g,
                    setImage: u,
                    nWidth: 184,
                    nHeight: 184,
                  }),
                ],
              }),
              (0, n.jsxs)("div", {
                className: ge().AboutCtn,
                children: [
                  (0, n.jsx)("div", {
                    className: "DialogLabel",
                    children: (0, p.we)("#EventEditor_AddSpeaker_About"),
                  }),
                  (0, n.jsx)(ee.Cl, {
                    value: c,
                    onChange: (e) => h(e.target.value),
                    rows: 8,
                    cols: 80,
                    nMinHeight: 40,
                    placeholder: (0, p.we)(
                      "#EventEditor_AddSpeaker_About_Placeholder",
                    ),
                  }),
                ],
              }),
              (0, n.jsxs)("div", {
                className: ge().PreviewCtn,
                children: [
                  (0, n.jsx)("div", {
                    className: "DialogLabel",
                    children: (0, p.we)("#Button_Preview"),
                  }),
                  (0, n.jsx)(ce.$k, {
                    company: r,
                    name: t,
                    title: s,
                    bioString: c,
                    photo: g ? g.url : void 0,
                  }),
                ],
              }),
            ],
          }),
        });
      };
      function ht(e) {
        const { friends: t, setSteamID: i } = e;
        return (0, n.jsx)("div", {
          className: ge().DropDownScroll,
          children: t.map((e) =>
            (0, n.jsx)(pt, { steamid: e, setSteamID: i }, e),
          ),
        });
      }
      function pt(e) {
        const { steamid: t, setSteamID: i } = e,
          { data: s } = (0, re.js)(t);
        return (0, n.jsx)(le.kt, {
          onSelected: () => i(new B.b(t)),
          children: (0, n.jsxs)("div", {
            style: { display: "flex", alignItems: "center" },
            children: [
              s &&
                (0, n.jsx)("img", {
                  className: ne().WhitelistAvatar,
                  src: s.avatar_url,
                }),
              s?.m_strPlayerName,
            ],
          }),
        });
      }
      let gt = class extends a.Component {
        m_isMounted = !1;
        m_bAppend = !1;
        constructor(e) {
          super(e), (this.state = { bPreserveNewLines: !1, strHTMLData: "" });
        }
        componentDidMount() {
          this.m_isMounted = !0;
        }
        componentWillUnmount() {
          this.m_isMounted = !1;
        }
        OnConvertAndOverriteHTML() {
          (this.m_bAppend = !1), this.ConvertBBCode();
        }
        OnConvertAndAppendHTML() {
          (this.m_bAppend = !0), this.ConvertBBCode();
        }
        async ConvertHtmlToBBCode(e, t) {
          let i = new URLSearchParams();
          i.append("content", e), i.append("preserve_newlines", t ? "1" : "0");
          let s = null;
          return (
            (s = await E().post(
              d.TS.COMMUNITY_BASE_URL + "/actions/ConvertHTMLToBBCode",
              i,
            )),
            s.data.content
          );
        }
        ConvertBBCode() {
          this.setState({ bConverting: !0 }),
            this.ConvertHtmlToBBCode(
              this.state.strHTMLData,
              this.state.bPreserveNewLines,
            )
              .then((e) => {
                this.m_isMounted &&
                  (this.m_bAppend
                    ? st.append(e, this.props.textareaRef.current)
                    : st.overwrite(e, this.props.textareaRef.current),
                  this.setState({ bConverting: !1, bFinishedConverting: !0 }));
              })
              .catch((e) => {
                (0, ie.pg)(
                  (0, n.jsx)(C.KG, {
                    strTitle: (0, p.we)("#EventEditor_ConvertHTML_Error"),
                    strDescription: (0, p.we)(
                      "#EventEditor_ConvertHTML_Error_Desc",
                      e.response && e.response.data ? e.response.data.msg : e,
                    ),
                    bAlertDialog: !0,
                    bDestructiveWarning: !0,
                  }),
                  this.props.ownerWindow,
                  { strTitle: (0, p.we)("#EventEditor_ConvertHTML_Error") },
                );
              });
        }
        OnCheckboxChange(e) {
          let t = e.target.checked;
          t != this.state.bPreserveNewLines &&
            this.setState({ bPreserveNewLines: t });
        }
        OnTextAreaChange(e) {
          this.setState({ strHTMLData: e.currentTarget.value });
        }
        render() {
          const { closeModal: e } = this.props;
          return this.state.bConverting
            ? (0, n.jsx)(C.o0, {
                strTitle: (0, p.we)("#EventEditor_ImportFromHTML"),
                strDescription: (0, p.we)(
                  "#EventEditor_ImportFromHTML_ConversionInProgress",
                ),
                closeModal: e,
                bAlertDialog: !0,
                onOK: e,
                onCancel: e,
                children: (0, n.jsx)(he.t, {}),
              })
            : this.state.bFinishedConverting
              ? (0, n.jsx)(C.o0, {
                  strTitle: (0, p.we)("#EventEditor_ImportFromHTML"),
                  strDescription: (0, p.we)(
                    "#EventEditor_ImportFromHTML_ConvertFinished",
                  ),
                  closeModal: e,
                  bAlertDialog: !0,
                  onOK: e,
                  onCancel: e,
                })
              : (0, n.jsx)(C.eV, {
                  title: (0, p.we)("#EventEditor_ImportFromHTML"),
                  onOK: this.OnConvertAndOverriteHTML,
                  onCancel: e,
                  className: ge().BBCodeEditorInputStyles,
                  children: (0, n.jsxs)(ee.nB, {
                    children: [
                      (0, n.jsx)(ee.a3, {
                        children: (0, n.jsxs)("div", {
                          className: (0, h.A)(
                            l().FlexColumnContainer,
                            ge().ImportHTMLCtn,
                          ),
                          children: [
                            (0, n.jsx)("div", {
                              className: l().FlexColumnContainer,
                              children: (0, p.PP)(
                                "#EventEditor_ImportFromHTML_ConvertDescription",
                                (0, n.jsx)("a", {
                                  target: d.TS.IN_CLIENT ? void 0 : "_blank",
                                  href: "https://partner.steamgames.com/doc/marketing/event_tools/import",
                                  children: (0, p.we)(
                                    "#EventEditor_ImportFromHTML_ConvertLearn",
                                  ),
                                }),
                              ),
                            }),
                            (0, n.jsx)("textarea", {
                              value: this.state.strHTMLData,
                              placeholder: (0, p.we)(
                                "#EventEditor_ImportFromHTML_Instruction",
                              ),
                              className: ge().ImportHTMLTextArea,
                              onChange: this.OnTextAreaChange,
                              ref: (e) => e && e.focus(),
                            }),
                            (0, n.jsxs)("div", {
                              className: ge().ImportHTMLCheckBoxLine,
                              children: [
                                (0, n.jsx)("input", {
                                  id: "ImportFromHTMLNewLines",
                                  type: "checkbox",
                                  checked: this.state.bPreserveNewLines,
                                  onChange: this.OnCheckboxChange,
                                }),
                                (0, n.jsxs)("label", {
                                  htmlFor: "ImportFromHTMLNewLines",
                                  children: [
                                    (0, p.we)(
                                      "#EventEditor_ImportFromHTML_PreserveNewlines",
                                    ),
                                    (0, n.jsx)(tt.o, {
                                      tooltip: (0, p.we)(
                                        "#EventEditor_ImportFromHTML_PreserveNewlines_Hint",
                                      ),
                                    }),
                                  ],
                                }),
                              ],
                            }),
                            (0, n.jsx)("div", {
                              children: (0, p.we)(
                                "#EventEditor_ImportFromHTML_ConvertToBBCode",
                              ),
                            }),
                          ],
                        }),
                      }),
                      (0, n.jsx)(ee.wi, {
                        children: (0, n.jsx)(ee.VQ, {
                          onCancel: e,
                          strOKText: (0, p.we)("#Button_Overwrite"),
                          onUpdate: this.OnConvertAndAppendHTML,
                          strUpdateText: (0, p.we)("#Button_Append"),
                        }),
                      }),
                    ],
                  }),
                });
        }
      };
      (0, s.Cg)([g.oI], gt.prototype, "OnConvertAndOverriteHTML", null),
        (0, s.Cg)([g.oI], gt.prototype, "OnConvertAndAppendHTML", null),
        (0, s.Cg)([g.oI], gt.prototype, "OnCheckboxChange", null),
        (0, s.Cg)([g.oI], gt.prototype, "OnTextAreaChange", null),
        (gt = (0, s.Cg)([x.PA], gt));
      var dt = i(9319),
        ut = i(83450),
        mt = i.n(ut);
      const It = (0, x.PA)((e) => {
        const { appid: t, eventLink: i } = e,
          s = a.useRef(null),
          { data: o } = (0, re.js)(d.iA.steamid),
          [r, c] = a.useState(""),
          [g, u] = a.useState(!1),
          [m, I] = a.useState(""),
          [L, T] = a.useState(!1);
        a.useEffect(
          () => () =>
            s.current &&
            s.current("ShareEventOnFriendsActivityFeed: unmounting"),
          [],
        );
        const w = o ? o.avatar_url : null,
          A = B.b.InitFromAccountID(d.iA.accountid),
          x = async () => {
            s.current &&
              s.current("ShareEventOnFriendsActivityFeed: cancel previous...");
            const e = E().CancelToken.source();
            (s.current = e.cancel), u(!0);
            let n = r;
            0 != n.trim().length && (n += "\n\n"), (n += i);
            const a =
              d.TS.COMMUNITY_BASE_URL +
              "profiles/" +
              A.ConvertTo64BitString() +
              "/ajaxpostuserstatus";
            try {
              if (d.TS.IN_STEAMUI) {
                let i = k.w.Init(R.kV);
                i.Body().set_appid(t), i.Body().set_status_text(n);
                let s = await R.xt.PostStatusToFriends(
                  O.Vw.CMInterface.GetServiceTransport(),
                  i,
                );
                if (1 != s.GetEResult()) {
                  const t =
                    (0, p.we)("#EventDisplay_Share_Failure") +
                    "\n\n" +
                    s.GetEResult();
                  return (
                    console.error(t, !e.token.reason),
                    void (e.token.reason || I(t))
                  );
                }
              } else {
                const i = new FormData();
                i.append("appid", "" + t),
                  i.append("status_text", n),
                  i.append("sessionid", d.TS.SESSIONID);
                const s = await E().post(a, i, { withCredentials: !0 });
                if (200 != s.status || 1 != s?.data?.success) {
                  const t =
                    (0, p.we)("#EventDisplay_Share_Failure") +
                    "\n\n" +
                    s?.data?.message;
                  return console.error(t), void (e.token.reason || I(t));
                }
              }
              e.token.reason ||
                (T(!0), I((0, p.we)("#EventDisplay_Share_Success")));
            } catch (t) {
              const i = (0, Oe.H)(t),
                s =
                  (0, p.we)("#EventDisplay_Share_Failure") +
                  "\n\n" +
                  i.strErrorMsg;
              e.token.reason || I(s), console.error(s);
            }
          };
        return g
          ? (0, n.jsx)(C.o0, {
              strDescription: "",
              strTitle: (0, p.we)("#Button_Share"),
              onCancel: e.closeModal,
              onOK: e.closeModal,
              bAlertDialog: !0,
              children: (0, n.jsxs)("div", {
                className: l().FlexColumnContainer,
                children: [
                  (0, n.jsx)("div", {
                    children: (0, p.we)(
                      "#EventDisplay_Share_OnMyStatus_Details",
                    ),
                  }),
                  (0, n.jsxs)("div", {
                    className: mt().Container,
                    children: [
                      0 == m?.length
                        ? (0, n.jsx)(he.t, { position: "center" })
                        : (0, n.jsx)("div", { children: m }),
                      Boolean(L) &&
                        (0, n.jsx)("a", {
                          href:
                            d.TS.COMMUNITY_BASE_URL +
                            "profiles/" +
                            A.ConvertTo64BitString() +
                            "/home",
                          target: d.TS.IN_CLIENT ? void 0 : "_blank",
                          children: (0, p.we)(
                            "#EventDisplay_Share_OpenActivityFeed",
                          ),
                        }),
                    ],
                  }),
                ],
              }),
            })
          : (0, n.jsx)(C.o0, {
              strDescription: "",
              strTitle: (0, p.we)("#Button_Share"),
              onCancel: e.closeModal,
              onOK: x,
              strOKButtonText: (0, p.we)("#Button_Post"),
              children: (0, n.jsxs)("div", {
                className: l().FlexColumnContainer,
                children: [
                  (0, n.jsx)("div", {
                    children: (0, p.we)(
                      "#EventDisplay_Share_OnMyStatus_Details",
                    ),
                  }),
                  (0, n.jsxs)("div", {
                    className: (0, h.A)(
                      mt().Container,
                      l().FlexColumnContainer,
                    ),
                    children: [
                      (0, n.jsxs)("div", {
                        children: [
                          (0, n.jsx)("img", {
                            className: mt().SmallAvatar,
                            src: w,
                            "data-miniprofile": "s" + d.iA.steamid,
                          }),
                          (0, n.jsx)("div", {
                            className: (0, h.A)(l().FlexColumnContainer),
                            children: (0, n.jsx)(it, {
                              strPlaceholder: (0, p.we)(
                                "#EventDisplay_Share_OnMyStatus_Placeholder",
                              ),
                              fnGetCurText: () => r,
                              fnOnTextChange: (e) => c(e.currentTarget.value),
                              fnSetText: c,
                              emoticonStore: e.emoticonStore,
                              bSupportHTMLImport: !1,
                              showFormatHelp: "UserStatusPublished",
                              limitBBCode: N.iH,
                              classNameForTextArea: mt().ShareDescription,
                              bEmbeddedInDialog: !0,
                            }),
                          }),
                        ],
                      }),
                      (0, n.jsx)("div", {
                        className: mt().ShareLink,
                        children: (0, n.jsx)(dt.fh, {
                          text: i,
                          event: null,
                          partnerEventStore: e.partnerEventStore,
                        }),
                      }),
                    ],
                  }),
                ],
              }),
            });
      });
      var Ct = i(30470);
      const Lt = (e) => {
        const t = a.createRef(),
          [i, s] = a.useState(""),
          o = a.createRef();
        return (0, n.jsxs)("div", {
          children: [
            (0, n.jsxs)("div", {
              className: (0, h.A)(l().FlexRowContainer, A().linkField),
              onClick: (e) => {
                t.current &&
                  t.current.ownerDocument.defaultView.navigator.clipboard
                    .writeText(t.current.value)
                    .then((e) => {
                      s((0, p.we)("#EventDisplay_Share_CopiedToClipboard"));
                    })
                    .catch((e) => {
                      s(
                        (0, p.we)(
                          "#EventDisplay_Share_FailedToCopyToClipboard",
                        ),
                      ),
                        console.error("Failed to copy link to clipboard:", e);
                    });
              },
              children: [
                (0, n.jsx)("span", {
                  className: A().LinkInputLabel,
                  children: (0, p.we)(
                    e.labelOverride
                      ? e.labelOverride
                      : "#EventDisplay_Share_Link",
                  ),
                }),
                (0, n.jsx)("textarea", {
                  className: A().LinkInput,
                  ref: t,
                  value: e.eventLink,
                  readOnly: !0,
                }),
                document.queryCommandSupported("copy") &&
                  (0, n.jsx)("div", {
                    className: (0, h.A)(l().Button, l().Icon, A().LinkButton),
                    title: (0, p.we)("#ToolTip_CopyLinkToClipboard"),
                    children: (0, n.jsx)(L.he, {
                      toolTipContent: (0, p.we)("#ToolTip_CopyLinkToClipboard"),
                      children: (0, n.jsx)("img", {
                        className: A().ClipboardIcon,
                        src: Ct.A,
                      }),
                    }),
                  }),
              ],
            }),
            (0, n.jsx)("div", {
              ref: o,
              className: A().ClipboardText,
              children: i,
            }),
          ],
        });
      };
      class Tt extends a.Component {
        state = { bShareOnSteamDialog: !1, bRequireLoginToShare: !1 };
        ShareOnSteamActivityFeed() {
          d.iA.logged_in
            ? this.setState({ bShareOnSteamDialog: !0 })
            : d.TS.IN_CLIENT
              ? console.log(
                  "ShareEventOnSocialMediaDialog: In Client: Cannot use login widget. We expect to be already logged in.",
                )
              : this.setState({ bRequireLoginToShare: !0 });
        }
        render() {
          const {
            eventLink: e,
            sharePageUrls: t,
            appid: i,
            closeModal: s,
            emoticonStore: a,
            partnerEventStore: o,
          } = this.props;
          return this.state.bRequireLoginToShare
            ? (0, n.jsx)(C.o0, {
                strTitle: (0, p.we)("#EventDisplay_Share_NotLoggedIn"),
                strDescription: (0, p.we)(
                  "#EventDisplay_Share_NotLoggedIn_Description",
                ),
                strOKButtonText: (0, p.we)("#MobileLogin_SignIn"),
                onCancel: this.props.closeModal,
                onOK: () => (0, c.vg)(),
              })
            : this.state.bShareOnSteamDialog
              ? (0, n.jsx)(It, {
                  eventLink: e,
                  appid: i,
                  emoticonStore: a,
                  partnerEventStore: o,
                  closeModal: s,
                })
              : (0, n.jsx)(C.o0, {
                  strDescription: "",
                  strTitle: (0, p.we)("#Button_Share"),
                  onCancel: this.props.closeModal,
                  onOK: this.props.closeModal,
                  bAlertDialog: !0,
                  modalClassName: "EventDisplay_Share_Dialog",
                  children: (0, n.jsx)(wt, {
                    eventLink: e,
                    sharePageUrls: t,
                    fnShareOnSteamActivityFeed: this.ShareOnSteamActivityFeed,
                  }),
                });
        }
      }
      (0, s.Cg)([g.oI], Tt.prototype, "ShareOnSteamActivityFeed", null);
      const wt = (e) => {
        const {
            eventLink: t,
            sharePageUrls: i,
            fnShareOnSteamActivityFeed: s,
          } = e,
          r = (0, d.Y2)(),
          [c, g] = (0, a.useState)((0, o.sf)(d.TS.LANGUAGE)),
          C = (0, a.useMemo)(() => {
            const e = new URL(t);
            return e.searchParams.set("l", (0, o.Lg)(c)), e.href;
          }, [c, t]);
        return (0, n.jsxs)("div", {
          className: (0, h.A)(l().FlexColumnContainer, A().share_controls_ctn),
          children: [
            !r &&
              (0, n.jsxs)(n.Fragment, {
                children: [
                  (0, n.jsxs)("div", {
                    className: A().ShareLanguagePicker,
                    children: [
                      (0, n.jsx)(ee.JU, {
                        className: A().LanguageLabel,
                        children: (0, p.we)(
                          "#EventDisplay_Share_LanguageLabel",
                        ),
                      }),
                      (0, n.jsx)("div", {
                        children: (0, n.jsx)(f, {
                          selectedLang: c,
                          fnOnLanguageChanged: g,
                        }),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: (0, h.A)(
                      l().FlexRowContainer,
                      A().ShareButtonContainer,
                    ),
                    style: { flexWrap: "wrap" },
                    children: [
                      (0, n.jsx)(L.he, {
                        toolTipContent: (0, p.we)(
                          "#EventDisplay_Share_OnSteam",
                        ),
                        children: (0, n.jsxs)("div", {
                          onClick: s,
                          className: (0, h.A)(
                            l().Button,
                            A().ShareBtn,
                            A().ShareSteamBtn,
                          ),
                          children: [
                            (0, n.jsx)("img", {
                              className: A().SteamIcon,
                              src: "data:image/svg+xml;base64,",
                            }),
                            (0, n.jsx)("span", {
                              style: { whiteSpace: "nowrap" },
                              children: (0, p.we)(
                                "#EventDisplay_Share_OnMyStatus",
                              ),
                            }),
                          ],
                        }),
                      }),
                      (0, n.jsx)(L.he, {
                        toolTipContent: (0, p.we)(
                          "#EventDisplay_Share_OnFaceBook",
                        ),
                        children: (0, n.jsx)(T.uU, {
                          href: i.strFacebookUrl,
                          className: A().ShareBtn,
                          children: (0, n.jsx)("img", {
                            className: (0, h.A)(l().Button),
                            src: u.A,
                          }),
                        }),
                      }),
                      (0, n.jsx)(L.he, {
                        toolTipContent: (0, p.we)(
                          "#EventDisplay_Share_OnTwitter",
                        ),
                        children: (0, n.jsx)(T.uU, {
                          href: i.strTwitterUrl,
                          className: A().ShareBtn,
                          children: (0, n.jsx)("img", {
                            className: (0, h.A)(l().Button),
                            src: I.A,
                          }),
                        }),
                      }),
                      (0, n.jsx)(L.he, {
                        toolTipContent: (0, p.we)(
                          "#EventDisplay_Share_OnReddit",
                        ),
                        children: (0, n.jsx)(T.uU, {
                          href: i.strRedditUrl,
                          className: A().ShareBtn,
                          children: (0, n.jsx)("img", {
                            className: (0, h.A)(l().Button),
                            src: m.A,
                          }),
                        }),
                      }),
                    ],
                  }),
                  (0, n.jsx)("div", { className: l().Divider }),
                ],
              }),
            (0, n.jsx)(Lt, { eventLink: C }),
          ],
        });
      };
    },
    29381: (e, t, i) => {
      "use strict";
      i.d(t, { I: () => I });
      var s,
        n = i(34629),
        a = i(62540),
        o = i(63696),
        r = i(51115),
        l = i(41230),
        c = i(79769),
        h = i(46108),
        p = i(64608),
        g = i(50376),
        d = i(21501),
        u = i.n(d),
        m = i(49412);
      !(function (e) {
        (e.topleft = "topleft"),
          (e.top = "top"),
          (e.topright = "topright"),
          (e.left = "left"),
          (e.middle = "middle"),
          (e.right = "right"),
          (e.bottomleft = "bottomleft"),
          (e.bottom = "bottom"),
          (e.bottomright = "bottomright");
      })(s || (s = {}));
      let I = class extends o.Component {
        m_rectLinkRegion;
        m_elLinkRegionBox;
        m_nLocalOffsetXPct;
        m_nLocalOffsetYPct;
        m_fnMouseUp = null;
        m_fnMouseMove = null;
        m_listeners = new c.Ji();
        m_strDescription = "";
        m_aspectRatio = 1;
        componentWillUnmount() {
          this.m_listeners.Unregister();
        }
        constructor(e) {
          super(e),
            (this.state = {
              curLeftPosPct: this.props.xPosPct,
              curTopPosPct: this.props.yPosPct,
              curRightPosPct: 100 - (this.props.widthPct + this.props.xPosPct),
              curBottomPosPct:
                100 - (this.props.yPosPct + this.props.heightPct),
              curWidthPct: this.props.widthPct,
              curHeightPct: this.props.heightPct,
              EdgeDown: void 0,
              text_link_url: this.props.link_url,
              text_link_description: this.props.link_description,
              bEditingLink: !1,
              valid_link: this.validateUrl(this.props.link_url),
            }),
            (this.m_strDescription = this.props.link_description ?? ""),
            (this.m_aspectRatio =
              this.props.heightPct > 0 && this.props.widthPct > 0
                ? this.props.widthPct / this.props.heightPct
                : 1);
        }
        LinkRegionBoxRef(e) {
          this.m_elLinkRegionBox = e;
        }
        OnMouseDown(e, t) {
          this.m_elLinkRegionBox?.parentElement &&
            this.m_elLinkRegionBox.ownerDocument.defaultView &&
            ((this.m_fnMouseUp = (e) => {
              this.OnMouseUp(e, t);
            }),
            (this.m_fnMouseMove = (e) => {
              this.OnMouseMove(e, t);
            }),
            this.setState({ EdgeDown: t }),
            (this.m_rectLinkRegion =
              this.m_elLinkRegionBox.parentElement.getBoundingClientRect()),
            (this.m_nLocalOffsetXPct =
              ((e.clientX - this.m_rectLinkRegion.left) /
                (this.m_rectLinkRegion.right - this.m_rectLinkRegion.left)) *
                100 -
              this.state.curLeftPosPct),
            (this.m_nLocalOffsetYPct =
              ((e.clientY - this.m_rectLinkRegion.top) /
                (this.m_rectLinkRegion.bottom - this.m_rectLinkRegion.top)) *
                100 -
              this.state.curTopPosPct),
            this.m_listeners.AddEventListener(
              this.m_elLinkRegionBox.ownerDocument.defaultView,
              "mousemove",
              this.m_fnMouseMove,
            ),
            this.m_listeners.AddEventListener(
              this.m_elLinkRegionBox.ownerDocument.defaultView,
              "mouseup",
              this.m_fnMouseUp,
            )),
            e.preventDefault(),
            e.stopPropagation();
        }
        OnMouseMove(e, t) {
          if (void 0 !== this.state.EdgeDown) {
            switch ((e.shiftKey && this.m_fnMouseUp(), t)) {
              case s.left:
                this.UpdateState({
                  curLeftPosPct: this.CalcLeftEdge(e.clientX),
                });
                break;
              case s.right:
                this.UpdateState({
                  curRightPosPct: this.CalcRightEdge(e.clientX),
                });
                break;
              case s.top:
                this.UpdateState({ curTopPosPct: this.CalcTopEdge(e.clientY) });
                break;
              case s.bottom:
                this.UpdateState({
                  curBottomPosPct: this.CalcBottomEdge(e.clientY),
                });
                break;
              case s.topleft:
                this.UpdateState({
                  curTopPosPct: this.CalcBottomEdge(e.clientY),
                  curLeftPosPct: this.CalcLeftEdge(e.clientX),
                });
                break;
              case s.topright:
                this.UpdateState({
                  curTopPosPct: this.CalcTopEdge(e.clientY),
                  curRightPosPct: this.CalcRightEdge(e.clientX),
                });
                break;
              case s.bottomleft:
                this.UpdateState({
                  curLeftPosPct: this.CalcLeftEdge(e.clientX),
                  curBottomPosPct: this.CalcBottomEdge(e.clientY),
                });
                break;
              case s.bottomright:
                this.UpdateState({
                  curRightPosPct: this.CalcRightEdge(e.clientX),
                  curBottomPosPct: this.CalcBottomEdge(e.clientY),
                });
                break;
              case s.middle: {
                const t = (0, m.OQ)(
                    this.CalcLeftEdge(e.clientX),
                    0,
                    100 - this.state.curWidthPct,
                  ),
                  i = 100 - (t + this.state.curWidthPct),
                  s = (0, m.OQ)(
                    this.CalcTopEdge(e.clientY),
                    0,
                    100 - this.state.curHeightPct,
                  ),
                  n = {
                    curLeftPosPct: t,
                    curRightPosPct: i,
                    curTopPosPct: s,
                    curBottomPosPct: 100 - (s + this.state.curHeightPct),
                  };
                this.setState(n);
                break;
              }
            }
            e.preventDefault(), e.stopPropagation();
          }
        }
        IsValidPct(e) {
          return e >= 0 && e <= 100;
        }
        UpdateState(e) {
          let t =
              void 0 !== e.curTopPosPct
                ? e.curTopPosPct
                : this.state.curTopPosPct,
            i =
              void 0 !== e.curBottomPosPct
                ? e.curBottomPosPct
                : this.state.curBottomPosPct,
            s =
              void 0 !== e.curLeftPosPct
                ? e.curLeftPosPct
                : this.state.curLeftPosPct,
            n =
              void 0 !== e.curRightPosPct
                ? e.curRightPosPct
                : this.state.curRightPosPct,
            a = (0, m.OQ)(
              100 - n - s,
              this.props.widthMinPct || 0,
              this.props.widthMaxPct || 100,
            ),
            o = (0, m.OQ)(
              100 - i - t,
              this.props.heightMinPct || 0,
              this.props.heightMaxPct || 100,
            );
          this.props.bLockAspectRatio &&
            (void 0 !== e.curLeftPosPct || void 0 !== e.curRightPosPct
              ? (o = a / this.m_aspectRatio)
              : (a = o * this.m_aspectRatio)),
            void 0 !== e.curLeftPosPct
              ? (s = 100 - n - a)
              : (n = 100 - (s + a)),
            void 0 !== e.curTopPosPct ? (t = 100 - i - o) : (i = 100 - (t + o));
          const r = 100 - n - s,
            l = 100 - i - t;
          this.IsValidPct(s) &&
            this.IsValidPct(n) &&
            this.IsValidPct(t) &&
            this.IsValidPct(i) &&
            this.IsValidPct(r) &&
            this.IsValidPct(l) &&
            this.setState({
              curLeftPosPct: s,
              curRightPosPct: n,
              curTopPosPct: t,
              curBottomPosPct: i,
            });
        }
        GetXPercent(e) {
          return this.m_rectLinkRegion
            ? ((e - this.m_rectLinkRegion.left) /
                (this.m_rectLinkRegion.right - this.m_rectLinkRegion.left)) *
                100 -
                (this.m_nLocalOffsetXPct ?? 0)
            : 0;
        }
        GetYPercent(e) {
          return this.m_rectLinkRegion
            ? ((e - this.m_rectLinkRegion.top) /
                (this.m_rectLinkRegion.bottom - this.m_rectLinkRegion.top)) *
                100 -
                (this.m_nLocalOffsetYPct ?? 0)
            : 0;
        }
        CalcLeftEdge(e) {
          return (0, m.OQ)(this.GetXPercent(e), 0, 100);
        }
        CalcRightEdge(e) {
          return (0, m.OQ)(
            100 - (this.GetXPercent(e) + this.state.curWidthPct),
            0,
            100,
          );
        }
        CalcTopEdge(e) {
          return (0, m.OQ)(this.GetYPercent(e), 0, 100);
        }
        CalcBottomEdge(e) {
          return (0, m.OQ)(
            100 - (this.GetYPercent(e) + this.state.curHeightPct),
            0,
            100,
          );
        }
        OnMouseUp(e, t) {
          this.setState({
            curWidthPct:
              100 - this.state.curRightPosPct - this.state.curLeftPosPct,
          }),
            this.setState({
              curHeightPct:
                100 - this.state.curBottomPosPct - this.state.curTopPosPct,
            }),
            this.setState({ EdgeDown: void 0 }),
            this.props.updateFn(this.props.index, {
              xPosPct: this.state.curLeftPosPct,
              yPosPct: this.state.curTopPosPct,
              widthPct: this.state.curWidthPct,
              heightPct: this.state.curHeightPct,
              link_url: this.state.text_link_url,
              link_description: this.state.text_link_description,
            }),
            this.m_listeners.Unregister();
        }
        async HandleDelete() {
          this.props.deleteFn && this.props.deleteFn(this.props.index);
        }
        OnSetLinkURLChange(e) {
          this.setState({
            text_link_url: e.target.value,
            valid_link: this.validateUrl(e.target.value),
          });
        }
        OnSetLinkDescriptionChange(e) {
          this.setState({ text_link_description: e.target.value });
        }
        validateUrl(e) {
          return (
            null != e &&
            /^https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/i.test(
              e,
            )
          );
        }
        OnSaveLink() {
          (this.m_strDescription = this.state.text_link_description ?? ""),
            this.setState({ bEditingLink: !this.state.bEditingLink }),
            this.props.updateFn(this.props.index, {
              xPosPct: this.state.curLeftPosPct,
              yPosPct: this.state.curTopPosPct,
              widthPct: this.state.curWidthPct,
              heightPct: this.state.curHeightPct,
              link_url: this.state.text_link_url,
              link_description: this.state.text_link_description,
            });
        }
        OnEditLink() {
          this.setState({ bEditingLink: !this.state.bEditingLink });
        }
        render() {
          let e = {
              left: this.state.curLeftPosPct + "%",
              top: this.state.curTopPosPct + "%",
              right: this.state.curRightPosPct + "%",
              bottom: this.state.curBottomPosPct + "%",
            },
            t = u().LinkRegionDragBox;
          return (
            null != this.state.EdgeDown &&
              (t += ` ${u().EdgeDown} ` + u()[this.state.EdgeDown]),
            (0, a.jsxs)("div", {
              className: t,
              style: e,
              ref: this.LinkRegionBoxRef,
              draggable: !1,
              children: [
                (0, a.jsxs)("div", {
                  className: u().LinkRegionGridBox,
                  children: [
                    (0, a.jsx)("div", {
                      className: `${u().LinkRegionEdge} ${u().TopLeft}`,
                      onMouseDown: (e) => {
                        this.OnMouseDown(e, s.topleft);
                      },
                      draggable: !1,
                    }),
                    (0, a.jsx)("div", {
                      className: `${u().LinkRegionEdge} ${u().Top}`,
                      onMouseDown: (e) => {
                        this.OnMouseDown(e, s.top);
                      },
                    }),
                    (0, a.jsx)("div", {
                      className: `${u().LinkRegionEdge} ${u().TopRight}`,
                      onMouseDown: (e) => {
                        this.OnMouseDown(e, s.topright);
                      },
                      draggable: !1,
                    }),
                    (0, a.jsx)("div", {
                      className: `${u().LinkRegionEdge} ${u().Left}`,
                      onMouseDown: (e) => {
                        this.OnMouseDown(e, s.left);
                      },
                      draggable: !1,
                    }),
                    (0, a.jsxs)("div", {
                      className: `${u().LinkRegionEdge} ${u().Middle}`,
                      onMouseDown: (e) => {
                        this.OnMouseDown(e, s.middle);
                      },
                      draggable: !1,
                      children: [
                        this.props.deleteFn &&
                          (0, a.jsx)("div", {
                            className: u().LinkRegionDelete,
                            onClick: this.HandleDelete,
                            children: (0, a.jsx)(g.sED, {}),
                          }),
                        !this.props.bDisableLink &&
                          (0, a.jsx)("div", {
                            className: u().LinkRegionSettings,
                            onClick: this.OnEditLink,
                            children: (0, a.jsx)(g.xv8, {}),
                          }),
                        (0, a.jsxs)("div", {
                          className: u().LinkText,
                          children: [" ", this.m_strDescription, " "],
                        }),
                      ],
                    }),
                    (0, a.jsx)("div", {
                      className: `${u().LinkRegionEdge} ${u().Right}`,
                      onMouseDown: (e) => {
                        this.OnMouseDown(e, s.right);
                      },
                      draggable: !1,
                    }),
                    (0, a.jsx)("div", {
                      className: `${u().LinkRegionEdge} ${u().BottomLeft}`,
                      onMouseDown: (e) => {
                        this.OnMouseDown(e, s.bottomleft);
                      },
                      draggable: !1,
                    }),
                    (0, a.jsx)("div", {
                      className: `${u().LinkRegionEdge} ${u().Bottom}`,
                      onMouseDown: (e) => {
                        this.OnMouseDown(e, s.bottom);
                      },
                      draggable: !1,
                    }),
                    (0, a.jsx)("div", {
                      className: `${u().LinkRegionEdge} ${u().BottomRight}`,
                      onMouseDown: (e) => {
                        this.OnMouseDown(e, s.bottomright);
                      },
                      draggable: !1,
                    }),
                  ],
                }),
                this.state.bEditingLink &&
                  (0, a.jsxs)("div", {
                    className: u().LinkRegionInfo,
                    children: [
                      (0, a.jsx)(p.pd, {
                        className: u().LinkRegionInput,
                        type: "text",
                        name: "link_url",
                        value: this.state.text_link_url,
                        label: (0, h.we)("#SteamTV_LinkURL"),
                        placeholder: "https://www.example.com",
                        onChange: this.OnSetLinkURLChange,
                        mustBeURL: !0,
                      }),
                      (0, a.jsx)(p.pd, {
                        className: u().LinkRegionInput,
                        type: "text",
                        name: "link_description",
                        value: this.state.text_link_description,
                        label: (0, h.we)("#SteamTV_LinkDescription"),
                        placeholder: (0, h.we)(
                          "#SteamTV_LinkDescription_Placeholder",
                        ),
                        onChange: this.OnSetLinkDescriptionChange,
                      }),
                      (0, a.jsxs)("div", {
                        className: u().LinkRegionButtonContainer,
                        children: [
                          (0, a.jsxs)(p.$n, {
                            disabled: !this.state.valid_link,
                            onClick: this.OnSaveLink,
                            children: [" ", (0, h.we)("#Button_OK"), " "],
                          }),
                          (0, a.jsxs)(p.$n, {
                            onClick: this.OnEditLink,
                            children: [" ", (0, h.we)("#Button_Cancel")],
                          }),
                        ],
                      }),
                    ],
                  }),
              ],
            })
          );
        }
      };
      (0, n.Cg)([r.oI], I.prototype, "LinkRegionBoxRef", null),
        (0, n.Cg)([r.oI], I.prototype, "OnMouseDown", null),
        (0, n.Cg)([r.oI], I.prototype, "OnMouseMove", null),
        (0, n.Cg)([r.oI], I.prototype, "OnMouseUp", null),
        (0, n.Cg)([r.oI], I.prototype, "HandleDelete", null),
        (0, n.Cg)([r.oI], I.prototype, "OnSetLinkURLChange", null),
        (0, n.Cg)([r.oI], I.prototype, "OnSetLinkDescriptionChange", null),
        (0, n.Cg)([r.oI], I.prototype, "OnSaveLink", null),
        (0, n.Cg)([r.oI], I.prototype, "OnEditLink", null),
        (I = (0, n.Cg)([l.PA], I));
    },
    30470: (e, t, i) => {
      "use strict";
      i.d(t, { A: () => s });
      const s =
        "data:image/svg+xml;base64,";
    },
    6367: (e, t, i) => {
      "use strict";
      i.d(t, { A: () => s });
      const s =
        "data:image/png;base64,";
    },
    99447: (e, t, i) => {
      "use strict";
      i.d(t, { A: () => s });
      const s =
        "data:image/png;base64,";
    },
    73674: (e, t, i) => {
      "use strict";
      i.d(t, { A: () => s });
      const s =
        "data:image/png;base64,";
    },
  },
]); //# sourceMappingURL=file:///home/buildbot/buildslave/npm-coordinator-deckard-hotfix/build/client/steamui/sourcemaps/9441.js.map
